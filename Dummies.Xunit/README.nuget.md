...
The [xUnit v3](https://xunit.net) companion of [Dummies](https://www.nuget.org/packages/Dummies).
...
`Dummies` makes a run reproducible by wrapping the test body in a delegate:
...
A failing test writes its seed to the test output:
    ...
    [Dummies] These arbitrary values were seeded with 1234. Reproduce this run with [Reproducible(Seed = 1234)].
...
- The seed is pinned through `Any.UseSeed(...)`, a public handle any test-framework
  adapter can use — this package holds no privileged access to `Dummies`.
...
- [Dummies](https://www.nuget.org/packages/Dummies)
