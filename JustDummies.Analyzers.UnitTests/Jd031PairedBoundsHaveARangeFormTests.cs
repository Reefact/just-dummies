...
public class Jd031PairedBoundsHaveARangeFormTests {
    ...
    public async Task Reports_a_string_chain_naming_the_range_form() {
        Diagnostic diagnostic = await SingleDiagnosticAsync("_ = Dummy.String().WithMinLength(8).WithMaxLength(20).Generate();");
    ...
    [Theory]
    [InlineData("_ = Dummy.ListOf(Dummy.Int32()).WithMinCount(1).WithMaxCount(5).Generate();", "WithCountBetween(1, 5)")]
    [InlineData("_ = Dummy.Int32().GreaterThanOrEqualTo(1).LessThanOrEqualTo(50).Generate();", "Between(1, 50)")]
    [InlineData("_ = Dummy.TimeSpan().GreaterThanOrEqualTo(TimeSpan.Zero).LessThanOrEqualTo(TimeSpan.FromHours(1)).Generate();", "Between(TimeSpan.Zero, TimeSpan.FromHours(1))")]
    [InlineData("_ = Dummy.DateTime().AfterOrEqualTo(DateTime.UnixEpoch).BeforeOrEqualTo(DateTime.MaxValue).Generate();", "Between(DateTime.UnixEpoch, DateTime.MaxValue)")]
    ...
    public async Task Reports_the_range_in_parameter_order_whatever_the_writing_order() {
        ...
        Diagnostic diagnostic = await SingleDiagnosticAsync("_ = Dummy.String().WithMaxLength(20).WithMinLength(8).Generate();");
    ...
    public async Task Reaches_a_floating_point_type_where_no_constant_folding_could() {
        ...
        Diagnostic diagnostic = await SingleDiagnosticAsync("_ = Dummy.Double().GreaterThanOrEqualTo(1.5).LessThanOrEqualTo(9.5).Generate();");
    ...
    public async Task Reports_a_pair_of_equal_bounds_as_the_range_and_never_as_the_exact_form() {
        ...
        Diagnostic diagnostic = await SingleDiagnosticAsync("_ = Dummy.String().WithMinLength(8).WithMaxLength(8).Generate();");

        Check.That(diagnostic.GetMessage()).IsEqualTo("These two bounds are the range WithLengthBetween(8, 8)");
    }

    [Theory]
    [InlineData("_ = Dummy.String().WithLengthBetween(8, 20).Generate();")]
    [InlineData("_ = Dummy.ListOf(Dummy.Int32()).WithCountBetween(1, 5).Generate();")]
    [InlineData("_ = Dummy.Int32().Between(1, 50).Generate();")]
    public async Task Does_not_report_a_chain_already_written_as_a_range(string body) {
        await NothingReportedAsync(body);
    }

    [Theory]
    [InlineData("_ = Dummy.Int32().GreaterThan(5).LessThan(10).Generate();")]
    [InlineData("_ = Dummy.Double().GreaterThan(1.5).LessThan(9.5).Generate();")]
    [InlineData("_ = Dummy.DateTime().After(DateTime.UnixEpoch).Before(DateTime.MaxValue).Generate();")]
    ...
    [Theory]
    [InlineData("_ = Dummy.Int32().GreaterThan(5).LessThanOrEqualTo(10).Generate();")]
    [InlineData("_ = Dummy.Int32().GreaterThanOrEqualTo(5).LessThan(10).Generate();")]
    [InlineData("_ = Dummy.DateTime().After(DateTime.UnixEpoch).BeforeOrEqualTo(DateTime.MaxValue).Generate();")]
    public async Task Does_not_report_a_mixed_strict_and_inclusive_pair(string body) {
        await NothingReportedAsync(body);
    }

    [Theory]
    [InlineData("_ = Dummy.String().WithMinLength(8).WithMinLength(10).WithMaxLength(20).Generate();")]
    [InlineData("_ = Dummy.String().WithMinLength(8).WithMaxLength(20).WithMaxLength(15).Generate();")]
    ...
    [Theory]
    [InlineData("_ = Dummy.String().WithLength(8).WithMinLength(3).WithMaxLength(20).Generate();")]
    [InlineData("_ = Dummy.ListOf(Dummy.Int32()).WithCount(3).WithMinCount(1).WithMaxCount(5).Generate();")]
    public async Task Does_not_report_a_chain_that_also_settles_an_exact_size(string body) {
        await NothingReportedAsync(body);
    }

    [Theory]
    [InlineData("_ = Dummy.String().NonEmpty().WithMaxLength(20).Generate();")]
    [InlineData("_ = Dummy.Int32().Positive().LessThanOrEqualTo(50).Generate();")]
    ...
    public async Task Does_not_report_a_chain_asserted_to_throw() {
        const string source = """
            ...
            public static class Sample {
                public static void M() {
                    Check2.ThatCode(() => Dummy.String().WithMinLength(8).WithMaxLength(20));
                }
            }
            """;
    ...
    public async Task Reports_each_chain_once() {
        ImmutableArray<Diagnostic> diagnostics = await AnalyzeAsync("""
                    _ = Dummy.String().Alpha().WithMinLength(8).InUpperCase().WithMaxLength(20).Generate();
            """);
