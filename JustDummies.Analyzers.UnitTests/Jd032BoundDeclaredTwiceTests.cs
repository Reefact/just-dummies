...
public class Jd032BoundDeclaredTwiceTests {
    ...
    public async Task Reports_a_minimum_declared_twice() {
        Diagnostic diagnostic = await SingleDiagnosticAsync("_ = Dummy.String().WithMinLength(8).WithMinLength(10).Generate();");
    ...
    public async Task Reports_both_writing_orders_alike() {
        ...
        // make on purpose.
        Diagnostic diagnostic = await SingleDiagnosticAsync("_ = Dummy.String().WithMinLength(10).WithMinLength(8).Generate();");
    ...
    [Theory]
    [InlineData("_ = Dummy.String().WithMaxLength(50).WithMaxLength(20).Generate();", "WithMaxLength — 50 then 20")]
    [InlineData("_ = Dummy.ListOf(Dummy.Int32()).WithMinCount(1).WithMinCount(3).Generate();", "WithMinCount — 1 then 3")]
    [InlineData("_ = Dummy.ListOf(Dummy.Int32()).WithMaxCount(9).WithMaxCount(5).Generate();", "WithMaxCount — 9 then 5")]
    [InlineData("_ = Dummy.Int32().GreaterThanOrEqualTo(1).GreaterThanOrEqualTo(5).Generate();", "GreaterThanOrEqualTo — 1 then 5")]
    [InlineData("_ = Dummy.Int32().LessThanOrEqualTo(90).LessThanOrEqualTo(50).Generate();", "LessThanOrEqualTo — 90 then 50")]
    [InlineData("_ = Dummy.Double().GreaterThanOrEqualTo(1.5).GreaterThanOrEqualTo(2.5).Generate();", "GreaterThanOrEqualTo — 1.5 then 2.5")]
    [InlineData("_ = Dummy.TimeSpan().LessThanOrEqualTo(TimeSpan.FromHours(2)).LessThanOrEqualTo(TimeSpan.FromHours(1)).Generate();", "LessThanOrEqualTo — TimeSpan.FromHours(2) then TimeSpan.FromHours(1)")]
    [InlineData("_ = Dummy.DateTime().AfterOrEqualTo(DateTime.UnixEpoch).AfterOrEqualTo(DateTime.Today).Generate();", "AfterOrEqualTo — DateTime.UnixEpoch then DateTime.Today")]
    ...
    [Theory]
    [InlineData("_ = Dummy.Int32().GreaterThan(1).GreaterThan(5).Generate();")]
    [InlineData("_ = Dummy.DateTime().Before(DateTime.MaxValue).Before(DateTime.Today).Generate();")]
    ...
    [Theory]
    [InlineData("_ = Dummy.String().NonEmpty().WithMinLength(8).Generate();")]
    [InlineData("_ = Dummy.Int32().Positive().GreaterThanOrEqualTo(5).Generate();")]
    ...
    [Theory]
    [InlineData("_ = Dummy.String().WithMinLength(8).WithMaxLength(20).Generate();")]
    [InlineData("_ = Dummy.Int32().GreaterThanOrEqualTo(1).LessThanOrEqualTo(50).Generate();")]
    public async Task Does_not_report_two_different_bounds(string body) {
        await NothingReportedAsync(body);
    }

    [Theory]
    [InlineData("_ = Dummy.String().WithLengthBetween(1, 50).WithMaxLength(20).Generate();")]
    [InlineData("_ = Dummy.Int32().Between(1, 50).Between(2, 20).Generate();")]
    ...
    public async Task Does_not_report_a_chain_asserted_to_throw() {
        const string source = """
            ...
            public static class Sample {
                public static void M() {
                    Check2.ThatCode(() => Dummy.String().WithMinLength(8).WithMinLength(10));
                }
            }
            """;
    ...
    public async Task Reports_a_chain_once_even_when_two_bounds_are_doubled() {
        ImmutableArray<Diagnostic> diagnostics = await AnalyzeAsync("""
                    _ = Dummy.String().WithMinLength(1).WithMinLength(8).WithMaxLength(90).WithMaxLength(20).Generate();
            """);
