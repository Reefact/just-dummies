...
/// <summary>
///     JD012 — reports a choice pool built from generators rather than values. <c>Dummy.OneOf(Dummy.Int32(), Dummy.Int32())</c>
///     compiles and infers <c>T = DummyInt32</c>, so the pool holds recipes and drawing from it yields a recipe rather
...
public sealed class GeneratorPooledAsValueAnalyzer : DiagnosticAnalyzer {
    ...
    private static void OnCompilationStart(CompilationStartAnalysisContext context) {
        ...
        if (symbols.Dummy is null || symbols.IDummy is null || symbols.DummyContext is null) { return; }
    ...
    private static void Analyze(OperationAnalysisContext context, KnownSymbols symbols) {
        ...
        if (!GeneratorFacts.IsGenerator(method.TypeArguments[0], symbols.IDummy!)) { return; }

        context.ReportDiagnostic(Diagnostic.Create(Descriptors.GeneratorPooledAsValue, invocation.Syntax.GetLocation(), method.Name));
    }

    // Both entry points are mirrored on Dummy and DummyContext (SurfaceParityTests enforces the mirror), so the rule must
    ...
    private static bool IsChoiceFactory(IMethodSymbol method, KnownSymbols symbols) {
        return SymbolEqualityComparer.Default.Equals(method.ContainingType, symbols.Dummy)
            || SymbolEqualityComparer.Default.Equals(method.ContainingType, symbols.DummyContext);
    }
