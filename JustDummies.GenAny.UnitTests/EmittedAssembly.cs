...
internal static class EmittedAssembly {
    ...
    internal static string? DrawFrom(CSharpCompilation compilation, string generator, int count) {
        ...
        if (errors.Count > 0) { return "it does not compile: " + string.Join("; ", errors.Take(3)); }
        ...
        if (!emitted.Success) {
            return "it did not emit: " + string.Join("; ", emitted.Diagnostics
        ...
        if (type is null) { return $"the emitted assembly carries no {generator}."; }
    ...
    ///     none of it.
    /// </remarks>
    private static string? Draw(Type generator, int count) {
        MethodInfo? generate = generator.GetMethod("Generate", Type.EmptyTypes);

        if (generate is null) { return $"{generator.Name} has no Generate()."; }
        ...
        try {
            ...
            return "new " + generator.Name + "() threw " + Describe(invocation.InnerException);
        }
        ...
        for (int draw = 0; draw < count; draw++) {
            try {
                ...
                return $"draw {draw + 1} of {count} threw " + Describe(invocation.InnerException);
            }
        }
