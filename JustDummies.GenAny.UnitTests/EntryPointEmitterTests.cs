...
public sealed class EntryPointEmitterTests {
    ...
    public void AStaticRoot() {
        ...
        GoldenFile.Approve("AnyOrder.Entry.Static", entry.File.SourceText);
        Check.That(entry.Call).IsEqualTo("Dummies.Order()");
    }

    [Fact(DisplayName = "An extension member on the library's own Any.")]
    public void AnExtensionMemberOnAny() {
        ScaffoldedEntryPoint entry = Emitted(Shapes.Order(), EntryPointOptions.OnAny);

        GoldenFile.Approve("AnyOrder.Entry.Any", entry.File.SourceText);
        Check.That(entry.Call).IsEqualTo("Any.Order()");
    }
    ...
    public void ARootMovedIntoItsOwnNamespace() {
        ...
        GoldenFile.Approve("AnyPattern.Entry.Moved", entry.File.SourceText);
    ...
    public void TheGlobalNamespace() {
        ScaffoldedEntryPoint entry = Emitted(Shapes.Session(), EntryPointOptions.OnAny);

        GoldenFile.Approve("AnySession.Entry.Any", entry.File.SourceText);
        Check.That(entry.Call).IsEqualTo("Any.Session()");
    }

    [Fact(DisplayName = "The file is named after the generator it reaches.")]
    public void TheFileIsNamedAfterTheGenerator() {
        Check.That(Emitted(Shapes.Order(), EntryPointOptions.OnAny).File.FileName).IsEqualTo("AnyOrder.Entry.cs");
    }
    ...
    public void AnEntryPointNeverCarriesATodo() {
        Check.That(Emitted(Shapes.OrderWithTodo(), EntryPointOptions.OnAny).File.ContainsTodo).IsFalse();
    }
    ...
    public void ANestedTargetIsReachedByItsOwnName() {
        ScaffoldPlan nested = new(new TargetType("Order.Line", "Shop.Domain", NamespaceStyle.FileScoped),
                                  "AnyLine",
                                  ["JustDummies"],
                                  [ScaffoldedParameter.DrawnFrom("sku", "string", "Any.String().NonEmpty()")]);
        ...
        Check.That(entry.File.SourceText).Contains("public static AnyLine Line() {");
    }
    ...
    public void APlanAndAKindAreBothRequired() {
        Check.ThatCode(() => EntryPointEmitter.Emit(null!, EntryPointOptions.OnAny)).Throws<ArgumentNullException>();
