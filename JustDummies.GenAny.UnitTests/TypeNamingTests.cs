...
public sealed class TypeNamingTests {
    ...
    [Fact(DisplayName = "A type is named by the pattern: Order becomes AnyOrder.")]
    public void ATypeIsNamedByThePattern() {
        ITypeSymbol order = TypeIn(Source, "Shop.Domain.Order");

        Check.That(TypeNaming.GeneratorNameFor(order, NamingOptions.Default)).IsEqualTo("AnyOrder");
    }

    // §3.2: a nested type scaffolds to a TOP-LEVEL generator named after the nested type alone, so
    // `dum generate Order.Line` writes AnyLine and not AnyOrderLine.
    [Fact(DisplayName = "A nested type is named after itself alone: Order.Line becomes AnyLine.")]
    public void ANestedTypeIsNamedAfterItselfAlone() {
        ITypeSymbol line = TypeIn(Source, "Shop.Domain.Order+Line");

        Check.That(TypeNaming.GeneratorNameFor(line, NamingOptions.Default)).IsEqualTo("AnyLine");
    }

    [Fact(DisplayName = "The v1.0 pattern is Any{Type}.")]
    public void TheDefaultPatternIsTheOneVersionOneDotZeroOffers() {
        Check.That(NamingOptions.Default.Pattern).IsEqualTo("Any" + NamingOptions.TypePlaceholder);
    }
