...
///     <c>Dummies.Order()</c> or <c>Any.Order()</c> rather than only as <c>new AnyOrder()</c>.
/// </summary>
/// <remarks>
...
///     <para>
///         It is also what keeps §4.4 intact. <see cref="EntryPointKind.Any" /> needs C# 14, which the generator
...
public static class EntryPointEmitter {
    ...
    ///     <c>AnyLine</c>: reading a name the plan supplies, not resolving anything of its own.
    /// </remarks>
    ...
    private static void WriteHeader(StringBuilder file, ScaffoldPlan plan, EntryPointOptions entryPoint) {
        string option = entryPoint.Kind == EntryPointKind.Any
                            ? "any"
        ...
        Line(file, entryPoint.Kind == EntryPointKind.Any
    ...
    /// <summary>
    ///     What this file has to open: the library for <see cref="EntryPointKind.Any" />, and the generator's
    ...
    private static IReadOnlyList<string> Usings(ScaffoldPlan plan, EntryPointOptions entryPoint, string? entryNamespace) {
        ...
        if (entryPoint.Kind == EntryPointKind.Any) { usings.Add("JustDummies"); }
    ...
    private static void WriteBody(StringBuilder file,
                                  ...
                                  string method,
                                  string indent) {
        if (entryPoint.Kind == EntryPointKind.Any) {
    ...
    ///     and a second <c>Any</c> declared here would hide the library's rather than extend it.
    /// </summary>
    private static void WriteExtension(StringBuilder file, ScaffoldPlan plan, string method, string indent) {
        Line(file, $"{indent}/// <summary>Hangs <c>Any.{method}()</c> off the library's own entry point.</summary>");
        Line(file, $"{indent}public static class {plan.GeneratorName}Entry {{");
        Line(file, string.Empty);
        Line(file, $"{indent}{Indent}extension(Any) {{");
        Line(file, string.Empty);
