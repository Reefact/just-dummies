...
public sealed class EntryPointOptions {
    ...
    /// </summary>
    /// <remarks>
    ///     A static class named <c>Any</c> in the developer's project does not extend the library's façade: C#
    ///     resolves a simple type name in the enclosing namespace before any <c>using</c>, so it hides it, and
    ///     <c>Any.Int32()</c> fails to compile with <c>CS0117</c> — verified. <see cref="EntryPointKind.Any" />
    ///     is what this name is asking for, and it is a different mechanism.
    /// </remarks>
    public const string ReservedRootName = "Any";
    ...
    /// <summary>The entry point hangs off the library's own façade — <c>Any.Order()</c>.</summary>
    public static EntryPointOptions OnAny { get; } = new(EntryPointKind.Any, rootName: null, namespaceOverride: null);
    ...
    /// <summary>What the developer writes before the type's name — <c>Dummies</c>, or <c>Any</c>.</summary>
    public string Root => Kind == EntryPointKind.Any ? ReservedRootName : RootName ?? string.Empty;
    ...
    public static EntryPointOptions OnStaticRoot(string rootName) {
        ...
        if (name == ReservedRootName) {
            throw new ArgumentException($"A static root named '{ReservedRootName}' hides the library's own façade "
                                      + "rather than extending it; ask for the Any kind instead.",
                                        nameof(rootName));
        }
