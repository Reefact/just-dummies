...
///         through every site that concatenated <c>"Any"</c> onto a type name.
///     </para>
/// </remarks>
public sealed class NamingOptions {
    ...
    /// <summary>
    ///     <c>Any{Type}</c>: the v1.0 pattern, and the default v1.1 keeps so that an existing project sees no
    ///     change when the option arrives.
    /// </summary>
    public static NamingOptions Default { get; } = new("Any" + TypePlaceholder);
