...
public sealed class EmittedCodeCompilesTests {
    ...
    private const string OpenParameterGolden = "DummyOrderWithTodo";

    /// <summary>
    ///     The other approved file that must NOT compile — a different reason, the same mechanism (§5.5).
    /// </summary>
    private const string RequiresVerificationGolden = "DummyOrderRequiringVerification";
    ...
    public static TheoryData<string, string> EntryPointGoldens => new() {
        { "DummyOrder", "DummyOrder.Entry.Static" },
        { "DummyOrder", "DummyOrder.Entry.Dummy" },
        { "DummyPattern", "DummyPattern.Entry.Moved" },
        { "DummySession", "DummySession.Entry.Dummy" }
    };
    ...
    public void TheExtensionMemberEntryPointNeedsCSharp14() {
        Check.That(ParseErrorsAt("DummyOrder.Entry.Dummy", LanguageVersion.CSharp13)).Not.IsEmpty();
        Check.That(ParseErrorsAt("DummyOrder.Entry.Dummy", LanguageVersion.CSharp14)).IsEmpty();
        Check.That(ParseErrorsAt("DummyPattern.Entry.Moved", LanguageVersion.CSharp7_3)).IsEmpty();
    }
    ...
    ///     says something is missing; <c>DummyOrderReference</c> says which type to run <c>dum generate</c> on.
    ...
    public void AComposedParameterWithNoGeneratorYetFailsTheBuild() {
        ...
        Check.That(emitted).Contains("reference: new DummyOrderReference()");
        Check.That(emitted).Not.Contains("DummyValidReference");
        ...
        Check.That(errors.Single().GetMessage()).Contains("DummyOrderReference");
    }
    ...
    public void AParameterRequiringVerificationFailsTheDevelopersBuildWithItsBaseIntact() {
        ...
        Check.That(text).Contains("return new DummyCustomer();");
    }
    ...
    [InlineData("DummyPattern")]
    [InlineData("DummySession")]
    ...
    public void AnOddParameterNameEmitsAFileThatCompiles(string declared, string identifier, string field) {
        ...
        Check.That(emitted).Contains($"IDummy<string> {identifier})");
