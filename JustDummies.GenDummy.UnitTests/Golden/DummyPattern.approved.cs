...
namespace Shop.Legacy {
    ...
    ///     <c>With…</c> overloads.
    /// </summary>
    public sealed partial class DummyPattern : IDummy<Pattern> {

        private readonly IDummy<string> _text;

        /// <summary>Creates the generator with a default recipe for every constructor parameter.</summary>
        public DummyPattern()
            : this(text: Dummy.String().NonEmpty()) { }

        private DummyPattern(IDummy<string> text) {
            _text = text;
        }

        /// <summary>Pins <c>text</c> to a fixed value.</summary>
        public DummyPattern WithText(string value) {
            return WithText(new FixedValue<string>(value));
        }

        /// <summary>Draws <c>text</c> from <paramref name="generator" />.</summary>
        public DummyPattern WithText(IDummy<string> generator) {
            return new DummyPattern(generator);
        }
