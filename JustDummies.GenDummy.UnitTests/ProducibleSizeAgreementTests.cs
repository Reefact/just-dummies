...
///     <c>DummyString.WithMaxLength</c> caps it, a leftover from before ADR-0076 unified the rule. The result was
...
public sealed class ProducibleSizeAgreementTests {
    ...
    public void TheEngineStopsExactlyWhereTheLibraryDoes() {
        ...
        Check.ThatCode(() => Dummy.String().WithMinLength(largest)).DoesNotThrow();
        Check.ThatCode(() => Dummy.String().WithMinLength(largest + 1)).Throws<ArgumentOutOfRangeException>();
    }
    ...
    public void TheCountFamilyStopsAtTheSameSize() {
        ...
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithMinCount(largest)).DoesNotThrow();
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithMinCount(largest + 1)).Throws<ArgumentOutOfRangeException>();
    }
    ...
    private static bool Declares(int size) {
        ...
        return parameter.Expression == $"Dummy.String().WithMinLength({size})";
    }
