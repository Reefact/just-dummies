...
public sealed class ScaffolderTests {
    ...
    public void AScaffoldProducesTheFile() {
        ...
        Check.That(outcome.File!.FileName).IsEqualTo("DummySubject.cs");
        Check.That(outcome.File.SourceText).Contains("public sealed partial class DummySubject : IDummy<Subject> {");
    ...
    ///     <c>DummyGenerationException.EnumDeclaresNoMembers</c> the moment such a generator is constructed —
    ///     before <c>Generate()</c> is ever reached. Emitting <c>Dummy.Enum&lt;Empty&gt;()</c> anyway would
    ...
    public void ANestedTypeScaffoldsATopLevelGenerator() {
        ...
        Check.That(outcome.File!.FileName).IsEqualTo("DummyLine.cs");
        Check.That(outcome.File.SourceText).Contains("public sealed partial class DummyLine : IDummy<Subject.Line> {");
    }
    ...
    public void AParameterTypeOutsideAnyNamespaceOpensNoUsing() {
        ...
        Check.That(outcome.File.SourceText).Contains("IDummy<Sku>");
    }
