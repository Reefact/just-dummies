...
/// </summary>
/// <remarks>
///     A domain type named <c>Pattern</c> scaffolds to <c>DummyPattern</c>, which inside its own namespace
...
public sealed class ShadowingTests {
    ...
    public void ANameTheLibraryUsesIsReported() {
        ...
        Check.That(outcome.Warnings[0].Subject).IsEqualTo("DummyPattern");
        Check.That(outcome.Warnings[0].Other).IsEqualTo("JustDummies.DummyPattern");
    }

    /// <summary>
    ///     Arity is part of a type's identity in C#, so the library's eight generic <c>Dummy*</c> names cannot be
    ///     shadowed by a scaffolded one.
    /// </summary>
    /// <remarks>
    ///     Which matters: <c>Set</c>, <c>List</c> and <c>Sequence</c> are ordinary domain nouns, and warning on
    ///     all forty <c>Dummy*</c> names would cry wolf on the eight that cannot collide. Verified against the
