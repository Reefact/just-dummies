...
public sealed class WorkedExampleTests {
    ...
    private const string Source = """
                                  ...
                                  public sealed class DummyCustomer : IDummy<Customer> {
                                      public Customer Generate() { return new Customer("name"); }
                                  }

                                  public sealed class DummyOrderReference : IDummy<OrderReference> {
    ...
    public void TheWorkedExampleResolvesAsWritten() {
        ...
        Check.That(plan.Parameters.Select(parameter => parameter.Expression))
             .ContainsExactly("new DummyOrderReference()",
                              "new DummyCustomer()",
                              "Dummy.Int32().Positive()",
                              "Dummy.Enum<OrderStatus>()",
                              "Dummy.ListOf(Dummy.String().NonEmpty())",
                              "Dummy.DateTime()");
    }

    // §6's own worked recap: `reference` and `customer` are both `DummyX`, each drawn through the generator its
    ...
    /// </remarks>
    [Fact(DisplayName = "The file it emits is the approved DummyOrder, byte for byte.")]
    public void TheFileItEmitsIsTheApprovedOne() {
        ScaffoldOutcome outcome = Scaffold();

        Check.That(outcome.File!.SourceText).IsEqualTo(GoldenFile.ApprovedTextOf("DummyOrder"));
    }
