...
public static class GeneratorEmitter {
    ...
    private static void WriteGenerator(StringBuilder file, ScaffoldPlan plan, string indent) {
        ...
        Line(file, $"{indent}public sealed partial class {plan.GeneratorName} : IDummy<{plan.Target.Name}> {{");
        Line(file, string.Empty);
    ...
    /// <summary>
    ///     The one sentence a developer on <c>Dummy.WithSeed(…)</c> needs, in the place they will read it. An
    ///     <c>DummyContext</c> carries its own random source and the emitted expressions all come from the static
    ...
    private static void WriteTypeDocumentation(StringBuilder file, ScaffoldPlan plan, string indent) {
        ...
        Line(file, $"{indent}///     <c>Dummy.WithSeed(...)</c> context, pass that context's generators through the");
    ...
    private static void WriteFactories(StringBuilder file, ScaffoldPlan plan, string indent) {
        ...
        foreach (ScaffoldedParameter parameter in plan.Parameters) {
            ...
            Line(file, $"{indent}private static IDummy<{parameter.TypeDisplay}> {parameter.FactoryMethodName}() {{");
    ...
    /// </summary>
    /// <remarks>
    ///     It exists because <c>Dummy.OneOf(value)</c> cannot do this job: it rejects null and it consumes a draw,
    ...
    private static void WriteFixedValue(StringBuilder file, string indent) {
        Line(file, $"{indent}private sealed class FixedValue<TValue> : IDummy<TValue> {{");
        Line(file, string.Empty);
    ...
    private static string GeneratorTypeOf(ScaffoldedParameter parameter) {
        return $"IDummy<{parameter.TypeDisplay}>";
    }
