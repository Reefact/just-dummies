...
/// <summary>
///     The base table of §5.2: a parameter's type in, an expression of type <c>IDummy&lt;T&gt;</c> out — or
...
///     written down. A project on the <c>netstandard2.0</c> asset resolves no <c>Dummy.DateOnly()</c>, and the
...
internal sealed class GeneratorFor {
    ...
    /// </summary>
    /// <remarks>
    ///     An interface row needs no adapter <b>where the whole generator is what binds</b>: <c>IDummy&lt;out T&gt;</c>
    ///     is covariant, so the <c>IDummy&lt;List&lt;T&gt;&gt;</c> that <c>Dummy.ListOf(…)</c> produces is already an
    ///     <c>IDummy&lt;IReadOnlyList&lt;T&gt;&gt;</c> (§14.5). Variance across reference conversions is also exactly
    ...
    ///         converts <c>IDummy&lt;X&gt;</c> to <c>IDummy&lt;Y&gt;</c> when <c>X</c> converts to <c>Y</c>; it says
    ...
    /// </summary>
    /// <remarks>
    ///     <c>Dummy.ListOf</c> produces a <c>List&lt;T&gt;</c>, <c>Dummy.SetOf</c> a <c>HashSet&lt;T&gt;</c> and
    ///     <c>Dummy.DictionaryOf</c> a <c>Dictionary&lt;K,V&gt;</c>; <c>Dummy.SequenceOf</c> produces the very
    ...
    ///         element row for that many different values. <c>Dummy.Enum&lt;Permission&gt;()</c> has three, and a
    ...
    private DrawnGenerator Scalar(INamedTypeSymbol type) {
        ...
        return DrawnGenerator.From($"Dummy.{factory}()", generator, Refinement(factory, generator));
    }
    ...
    ///     <c>new DummyX()</c> and opens the composed type's own namespace (ADR-0062, and the only namespace ever
    ...
    ///     instead — <c>Dummy.String().NonEmpty().As(OrderReference.Create)</c> — would write one copy of that
    ...
    private DrawnGenerator Composed(INamedTypeSymbol type, Provenance refusal) {
        ...
        // told to write DummyRepository and neither would be the name to write. §5.5 still answers there.
    ...
    /// </summary>
    /// <remarks>
    ///     <c>Dummy.String()</c> unconstrained draws zero to sixteen letters and digits — it can return the empty
    ...
    /// <summary>
    ///     The base-table row for an enum type — <c>Dummy.Enum&lt;T&gt;()</c>, unless <paramref name="type" />
    ///     declares no member at all.
    /// </summary>
    /// <remarks>
    ///     The library draws only from an enum's declared members, never from an undeclared numeric value
    ///     (§14.6), and a type with none throws <c>DummyGenerationException.EnumDeclaresNoMembers</c> the moment
    ...
    private DrawnGenerator Enum(INamedTypeSymbol type) {
        ...
        return generator is null
                   ...
                   : DrawnGenerator.From($"Dummy.Enum<{names.Of(type)}>()", generator);
    }
    ...
    private DrawnGenerator Collection(string factory,
        ...
        return item is null
                   ? DrawnGenerator.Unresolved()
                   : DrawnGenerator.From($"Dummy.{factory}({item})", generator);
    }
    ...
    private DrawnGenerator Dictionary(INamedTypeSymbol type, int remaining, IReadOnlyCollection<ITypeSymbol> underway) {
        ...
        return keys is null || values is null
                   ? DrawnGenerator.Unresolved()
                   : DrawnGenerator.From($"Dummy.DictionaryOf({keys}, {values})", generator);
    }
    ...
    /// </summary>
    /// <remarks>
    ///     Variance in C# applies across reference conversions only. <c>IDummy&lt;string&gt;</c> is an
    ///     <c>IDummy&lt;string?&gt;</c> and needs nothing; <c>IDummy&lt;int&gt;</c> is <b>not</b> an
    ///     <c>IDummy&lt;int?&gt;</c>, so the conversion has to be written. Never <c>.OrNull()</c>: the emitted
