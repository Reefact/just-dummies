...
internal static class Guards {
    ...
    ///     reported as absent — which is the difference between <c>Dummy.String().NonEmpty()</c> and a plain
    ///     <c>Dummy.String()</c> for most of the types this tool exists to scaffold.
    /// </remarks>
    ...
    private static bool TryRecognise(ExpressionSyntax condition,
        ...
        if (condition is PrefixUnaryExpressionSyntax { RawKind: (int)SyntaxKind.LogicalNotExpression } negation) {
            // `!Enum.IsDefined(typeof(E), p)` — Dummy.Enum<E>() already draws only declared members, which holds
            // only where the parameter IS E. On an int-backed status column the row's own justification fails:
            // nothing narrows Dummy.Int32(), and the outcome is indistinguishable from a guard there was nothing
