...
/// <summary>
///     Named-case coverage for <see cref="DummyChar" /> that a property cannot express: the shape of the
...
public sealed class DummyCharTests {
    ...
    private static IEnumerable<char> Samples(IDummy<char> generator) {
    ...
    public void UnconstrainedDrawIsTheWholeOfAscii() {
        List<char> values = Samples(Dummy.Char()).ToList();
    ...
    public void ASecondFamilyConflictNamesBothSides() {
        Check.ThatCode(() => Dummy.Char().Alpha().Numeric())
    ...
    public void ASecondCasingConflictNamesBothSides() {
        Check.ThatCode(() => Dummy.Char().InLowerCase().InUpperCase())
             .Throws<ConflictingDummyConstraintException>()
