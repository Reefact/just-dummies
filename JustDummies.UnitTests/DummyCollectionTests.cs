...
public sealed class AnyCollectionTests {
    ...
    public void ListOfUnconstrained() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            List<int> list = Dummy.ListOf(Dummy.Int32().Between(1, 9)).Generate();
    ...
    public void ListOfCountFamily() {
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.ListOf(Dummy.Int32()).WithCount(5).Generate().Count).IsEqualTo(5);
            Check.That(Dummy.ListOf(Dummy.Int32()).Empty().Generate().Count).IsEqualTo(0);
            Check.That(Dummy.ListOf(Dummy.Int32()).NonEmpty().Generate().Count).IsStrictlyGreaterThan(0);
            Check.That(Dummy.ListOf(Dummy.Int32()).WithMinCount(3).Generate().Count).IsGreaterOrEqualThan(3);
            Check.That(Dummy.ListOf(Dummy.Int32()).WithMaxCount(2).Generate().Count).IsLessOrEqualThan(2);

            int ranged = Dummy.ListOf(Dummy.Int32()).WithCountBetween(4, 6).Generate().Count;
    ...
    public void ListOfCountConflicts() {
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithCount(3).WithMinCount(5))
             .Throws<ConflictingDummyConstraintException>()
             .WhichMember(conflict => conflict.Message).Contains("WithCount(3)");

        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithMinCount(5).WithMaxCount(3)).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithCount(2).WithCount(3)).Throws<ConflictingDummyConstraintException>();
    }

    [Fact(DisplayName = "ListOf: count constraints validate their arguments.")]
    public void ListOfCountValidation() {
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithCount(-1)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithMinCount(-1)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithCountBetween(6, 4)).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.ListOf<int>(null!)).Throws<ArgumentNullException>();
    }
    ...
    public void ProducedCountsAreCeilinged() {
        ...
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithCount(1_000_001)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithMinCount(1_000_001)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.SetOf(Dummy.Int32()).WithCountBetween(1_000_001, 2_000_000)).Throws<ArgumentOutOfRangeException>();

        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithCount(1_000_000)).DoesNotThrow();
    }
    ...
    public void AnEnormousCountNamesTheCallersParameter() {
        ...
        ArgumentOutOfRangeException error = Assert.Throws<ArgumentOutOfRangeException>(() => Dummy.ListOf(Dummy.Int32()).WithCount(int.MaxValue));
    ...
    public void AMaximumSteersTheCount() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.ListOf(Dummy.Int32()).WithMaxCount(50).Generate().Count).IsLessOrEqualThan(50);
        }

        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithMaxCount(int.MaxValue)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.ArrayOf(Dummy.Int32()).WithMaxCount(4_000_000)).Throws<ArgumentOutOfRangeException>();
    }

    [Fact(DisplayName = "Distinct: a wide-domain distinct list holds only distinct elements.")]
    public void DistinctOverAWideDomain() {
        for (int i = 0; i < SampleCount; i++) {
            List<int> list = Dummy.ListOf(Dummy.Int32().Between(1, 1000)).WithCount(20).Distinct().Generate();
    ...
    public void DistinctCardinalityConflictsBeforeDrawing() {
        Check.ThatCode(() => Dummy.SetOf(Dummy.Boolean()).WithCount(3).Generate())
             .Throws<ConflictingDummyConstraintException>()
             .WhichMember(fromBool => fromBool.Message).Contains("2 distinct value");

        Check.ThatCode(() => Dummy.SetOf(Dummy.Enum<Suit>()).WithMinCount(5).Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.SetOf(Dummy.Int32().Between(1, 3)).WithCount(5).Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32().Between(1, 3)).WithCount(5).Distinct().Generate()).Throws<ConflictingDummyConstraintException>();
        // Order-independent: turning distinct on after the count is set conflicts just the same.
        Check.ThatCode(() => Dummy.ListOf(Dummy.Boolean()).WithCount(3).Distinct().Generate()).Throws<ConflictingDummyConstraintException>();
    }
    ...
    public void DistinctFallbackThrowsAtGeneration() {
        ...
        IDummy<int> opaque = Dummy.Int32().Between(1, 3).As(value => value);

        Check.ThatCode(() => Dummy.SetOf(opaque).WithCount(5).Generate()).Throws<DummyGenerationException>();
    }
    ...
    public void DistinctOverReferenceEqualityIsInert() {
        ...
        List<Percentage> percentages = Dummy.ListOf(Dummy.Int32().Between(1, 2).As(Percentage.Create)).Distinct().WithCount(6).Generate();
    ...
    public void SetOfIsDistinct() {
        for (int i = 0; i < SampleCount; i++) {
            HashSet<int> set = Dummy.SetOf(Dummy.Int32().Between(1, 500)).WithCount(10).Generate();
    ...
    public void SetOfHonoursAComparer() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            HashSet<int> set = Dummy.SetOf(Dummy.Int32().Between(0, 999), modTen).WithCount(5).Generate();
        ...
        Check.ThatCode(() => Dummy.SetOf(Dummy.Int32().Between(0, 999), modTen).WithCount(20).Generate()).Throws<DummyGenerationException>();
    }
    ...
    public void ContainingPlacesValues() {
        for (int i = 0; i < SampleCount; i++) {
            List<int> list = Dummy.ListOf(Dummy.Int32().Between(1, 9)).WithCount(5).Containing(777).Generate();
            Check.That(list).Contains(777);
            Check.That(list.Count).IsEqualTo(5);
        }

        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32()).WithCount(1).Containing(1).Containing(2)).Throws<ConflictingDummyConstraintException>();

        Check.ThatCode(() => Dummy.SetOf(Dummy.Int32()).Containing(7).Containing(7).Generate())
             .Throws<ConflictingDummyConstraintException>()
    ...
    public void ContainingFromAGenerator() {
        for (int i = 0; i < SampleCount; i++) {
            List<int> list = Dummy.ListOf(Dummy.Int32().Between(1, 9)).NonEmpty().ContainingAny(Dummy.Int32().OneOf(4242)).Generate();
    ...
    public void ContainingOutsideDomainExtendsCardinality() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            HashSet<int> set = Dummy.SetOf(Dummy.Int32().OneOf(1, 2)).Containing(3).WithCount(3).Generate();
        ...
        // through DummyDictionary.ContainingKey (see DictionaryContainingKeyOutsideDomainExtendsCardinality).
        for (int i = 0; i < SampleCount; i++) {
            HashSet<int> list = [.. Dummy.ListOf(Dummy.Int32().OneOf(1, 2)).Containing(3).WithCount(3).Distinct().Generate()];
            Check.That(list).Contains(1, 2, 3);

            HashSet<int> quad = Dummy.SetOf(Dummy.Int32().OneOf(1, 2)).Containing(3).Containing(4).WithCount(4).Generate();
    ...
    public void ContainingInsideDomainDoesNotInflate() {
        ...
        Check.ThatCode(() => Dummy.SetOf(Dummy.Int32().OneOf(1, 2)).Containing(1).WithCount(3).Generate())
             .Throws<ConflictingDummyConstraintException>()
        ...
        Check.ThatCode(() => Dummy.SetOf(Dummy.Int32().OneOf(1, 2)).Containing(1).Containing(5).WithCount(4).Generate()).Throws<ConflictingDummyConstraintException>();
        for (int i = 0; i < SampleCount; i++) {
            HashSet<int> set = Dummy.SetOf(Dummy.Int32().OneOf(1, 2)).Containing(1).Containing(5).WithCount(3).Generate();
    ...
    public void EffectiveCardinalityIsOrderIndependent() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            Check.That(new HashSet<int>(Dummy.ListOf(Dummy.Int32().OneOf(1, 2)).WithCount(3).Containing(3).Distinct().Generate())).Contains(1, 2, 3);
            Check.That(new HashSet<int>(Dummy.ListOf(Dummy.Int32().OneOf(1, 2)).Distinct().Containing(3).WithCount(3).Generate())).Contains(1, 2, 3);
            Check.That(new HashSet<int>(Dummy.ListOf(Dummy.Int32().OneOf(1, 2)).Containing(3).WithCount(3).Distinct().Generate())).Contains(1, 2, 3);
        }

        // And rejected whatever the order, because the contained value is inside the domain.
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32().OneOf(1, 2)).WithCount(3).Containing(1).Distinct().Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32().OneOf(1, 2)).Distinct().Containing(1).WithCount(3).Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.ListOf(Dummy.Int32().OneOf(1, 2)).Containing(1).WithCount(3).Distinct().Generate()).Throws<ConflictingDummyConstraintException>();
    }
    ...
    public void AWideningConstraintIsHonouredWhereverItWasDeclared() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.SetOf(Dummy.Int32().Between(1, 3)).WithCount(4).Containing(99).Generate()).Contains(99);
            Check.That(Dummy.SetOf(Dummy.Int32().Between(1, 3)).WithCount(4).ContainingAny(Dummy.Int32().Between(50, 60)).Generate().Count).IsEqualTo(4);
            Check.That(Dummy.ListOf(Dummy.Int32().Between(1, 2)).Distinct().WithCount(3).Containing(99).Generate()).Contains(99);
            Check.That(Dummy.ListOf(Dummy.Int32().Between(1, 2)).WithCount(3).Distinct().Containing(99).Generate()).Contains(99);
            Check.That(Dummy.ListOf(Dummy.Int32().Between(1, 2)).Distinct().WithCount(3).ContainingAny(Dummy.Int32().Between(50, 60)).Generate().Count).IsEqualTo(3);
        }
    }
    ...
    public void AFinerComparerIsHonouredWhenDeclaredLast() {
        ...
        List<DateTimeOffset> pinned = Dummy.ListOf(Dummy.DateTimeOffset())
                                         .Distinct()
                                         .Containing(noon)
        ...
        Check.That(Dummy.ListOf(ranged).Distinct().WithCount(6).Distinct(new BySpellingComparer()).Generate().Count).IsEqualTo(6);
    }
    ...
    public void AComparerStricterThanTheDefaultDoesNotCauseAFalseConflict() {
        ...
        List<Tag> list = Dummy.ListOf(Dummy.OneOf(pooled))
    ...
    public void TheDefaultComparerStillRefusesAnInDomainPinnedValue() {
        ...
        Check.ThatCode(() => Dummy.ListOf(Dummy.OneOf(pooled)).Distinct().Containing(pinned).WithCount(2).Generate())
             .Throws<ConflictingDummyConstraintException>();
    }
    ...
    public void AComparerFinerThanTheDefaultDoesNotRefuseAnOffsetRange() {
        ...
        DummyDateTimeOffset ranged = Dummy.DateTimeOffset().Between(instant, instant).WithOffsetBetween(TimeSpan.FromHours(-2), TimeSpan.FromHours(2));

        List<DateTimeOffset> list = Dummy.ListOf(ranged).Distinct(new BySpellingComparer()).WithCount(3).Generate();
    ...
    public void TheDefaultComparerStillRefusesThreeSpellingsOfOneInstant() {
        ...
        Check.ThatCode(() => Dummy.ListOf(ranged).Distinct().WithCount(3).Generate())
             .Throws<ConflictingDummyConstraintException>();
    }
    ...
    public void APoolKeepsItsBoundUnderAFinerComparer() {
        ...
        DummyDateTimeOffset pooled = Dummy.DateTimeOffset().OneOf(first, second).WithOffsetBetween(TimeSpan.FromHours(-2), TimeSpan.FromHours(2));

        Check.ThatCode(() => Dummy.ListOf(pooled).Distinct(new BySpellingComparer()).WithCount(3).Generate())
             .Throws<ConflictingDummyConstraintException>();
    }
    ...
    public void ContainingNearMaximumCardinalityDoesNotOverflow() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            HashSet<long> set = Dummy.SetOf(Dummy.Int64().Between(0, long.MaxValue - 1)).Containing(-1L).WithCount(3).Generate();
    ...
    public void ContainingAnyDefersToGeneration() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            HashSet<int> set = Dummy.SetOf(Dummy.Int32().OneOf(1, 2)).ContainingAny(Dummy.Int32().GreaterThan(100)).WithCount(3).Generate();
            Check.That(set.Count).IsEqualTo(3);
        ...
        // overlap is opaque, so it is caught while drawing (a replayable DummyGenerationException) rather than as a
        // false eager conflict.
        Check.ThatCode(() => Dummy.SetOf(Dummy.Boolean()).ContainingAny(Dummy.Boolean()).ContainingAny(Dummy.Boolean()).ContainingAny(Dummy.Boolean()).Generate())
    ...
    public void ContainingUnderAMergingComparer() {
        ...
        // generation succeeds.
        for (int i = 0; i < SampleCount; i++) {
            HashSet<int> set = Dummy.SetOf(Dummy.Int32().OneOf(1, 2, 3), modTen).Containing(15).WithCount(4).Generate();
        ...
        Check.ThatCode(() => Dummy.SetOf(Dummy.Int32().OneOf(1, 2), modTen).Containing(12).WithCount(3).Generate()).Throws<DummyGenerationException>();
    }
    ...
    public void FiniteScalarGeneratorsGateEagerly() {
        ...
        Check.ThatCode(() => Dummy.SetOf(Dummy.Decimal().OneOf(1m, 2m)).WithCount(3).Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.SetOf(Dummy.Double().OneOf(1d, 2d)).WithCount(3).Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.SetOf(Dummy.Single().OneOf(1f, 2f)).WithCount(3).Generate()).Throws<ConflictingDummyConstraintException>();
#if NET8_0_OR_GREATER
        Check.ThatCode(() => Dummy.SetOf(Dummy.Int128().Between(1, 3)).WithCount(5).Generate()).Throws<ConflictingDummyConstraintException>();
#endif

        // Membership travels with cardinality: an out-of-domain contained value extends the effective domain...
        for (int i = 0; i < SampleCount; i++) {
            HashSet<decimal> set = Dummy.SetOf(Dummy.Decimal().OneOf(1m, 2m)).Containing(3m).WithCount(3).Generate();
        ...
        Check.ThatCode(() => Dummy.SetOf(Dummy.Decimal().OneOf(1m, 2m)).Containing(1m).WithCount(3).Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.SetOf(Dummy.Double().OneOf(1d, 2d)).Containing(2d).WithCount(3).Generate()).Throws<ConflictingDummyConstraintException>();
    }
    ...
    public void SingletonScalarDomainsGateEagerly() {
        ...
        Check.ThatCode(() => Dummy.SetOf(Dummy.Decimal().Zero()).WithCount(2).Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.SetOf(Dummy.Double().Between(1d, 1d)).WithCount(2).Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.SetOf(Dummy.Single().Zero()).WithCount(2).Generate()).Throws<ConflictingDummyConstraintException>();

        // The singleton still generates at count one, and an out-of-domain contained value extends it as usual.
        for (int i = 0; i < SampleCount; i++) {
            HashSet<decimal> one = Dummy.SetOf(Dummy.Decimal().Zero()).WithCount(1).Generate();
            Check.That(one).ContainsExactly(0m);

            HashSet<decimal> two = Dummy.SetOf(Dummy.Decimal().Zero()).Containing(5m).WithCount(2).Generate();
    ...
    public void ArrayOfProducesArrays() {
        for (int i = 0; i < SampleCount; i++) {
            int[] array = Dummy.ArrayOf(Dummy.Int32().Between(1, 100)).WithCount(6).Distinct().Generate();
    ...
    public void SequenceOfIsMaterialized() {
        IEnumerable<int> sequence = Dummy.SequenceOf(Dummy.Int32()).WithCount(5).Generate();
    ...
    public void DictionaryOfBehaves() {
        for (int i = 0; i < SampleCount; i++) {
            Dictionary<int, string> dictionary = Dummy.DictionaryOf(Dummy.Int32().Between(1, 1000), Dummy.String().NonEmpty()).WithCount(8).Generate();
            Check.That(dictionary.Count).IsEqualTo(8);
            Check.That(dictionary.Values).ContainsOnlyElementsThatMatch(value => value.Length > 0);
        }

        Check.ThatCode(() => Dummy.DictionaryOf(Dummy.Boolean(), Dummy.Int32()).WithCount(3).Generate()).Throws<ConflictingDummyConstraintException>();
        Check.ThatCode(() => Dummy.DictionaryOf<int, int>(null!, Dummy.Int32())).Throws<ArgumentNullException>();
    }
    ...
    public void DictionaryContainingKeyOutsideDomainExtendsCardinality() {
        ...
        // satisfiable — the out-of-domain cardinality-credit path DummyDictionary could not exercise before.
        for (int i = 0; i < SampleCount; i++) {
            Dictionary<int, string> dictionary =
                Dummy.DictionaryOf(Dummy.Int32().OneOf(1, 2), Dummy.String().NonEmpty()).ContainingKey(3).WithCount(3).Generate();
    ...
    public void DictionaryContainingKeyInsideDomainDoesNotInflate() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            Dictionary<int, string> dictionary =
                Dummy.DictionaryOf(Dummy.Int32().Between(1, 9), Dummy.String().NonEmpty()).WithCount(5).ContainingKey(7).Generate();
        ...
        Check.ThatCode(() => Dummy.DictionaryOf(Dummy.Int32().OneOf(1, 2), Dummy.String().NonEmpty()).ContainingKey(1).WithCount(3).Generate())
             .Throws<ConflictingDummyConstraintException>()
             .WhichMember(conflict => conflict.Message).Contains("2 distinct value");
    }
    ...
    public void DictionaryContainingAnyKeyForcesADrawnKey() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            Dictionary<int, string> dictionary =
                Dummy.DictionaryOf(Dummy.Int32().Between(1, 9), Dummy.String().NonEmpty()).NonEmpty().ContainingAnyKey(Dummy.Int32().OneOf(4242)).Generate();
            Check.That(dictionary.ContainsKey(4242)).IsTrue();
        }

        Check.ThatCode(() => Dummy.DictionaryOf(Dummy.Int32(), Dummy.Int32()).ContainingAnyKey(null!)).Throws<ArgumentNullException>();
    }
    ...
    public void DictionaryContainingEntryPinsTheValue() {
        for (int i = 0; i < SampleCount; i++) {
            ...
            Dictionary<int, int> dictionary =
                Dummy.DictionaryOf(Dummy.Int32().OneOf(1, 2), Dummy.Int32().Between(1, 9))
    ...
    public void DictionaryContainingEntryDuplicateKeyConflicts() {
        Check.ThatCode(() => Dummy.DictionaryOf(Dummy.Int32(), Dummy.Int32()).ContainingEntry(1, 10).ContainingEntry(1, 20).Generate())
             .Throws<ConflictingDummyConstraintException>();

        Check.ThatCode(() => Dummy.DictionaryOf(Dummy.Int32(), Dummy.Int32()).ContainingKey(1).ContainingEntry(1, 20).Generate())
             .Throws<ConflictingDummyConstraintException>();
    }
    ...
    public void PairAndTriple() {
        for (int i = 0; i < SampleCount; i++) {
            (int first, string second) pair = Dummy.PairOf(Dummy.Int32().Positive(), Dummy.String().NonEmpty()).Generate();
            Check.That(pair.first).IsStrictlyGreaterThan(0);
            Check.That(pair.second).IsNotEmpty();

            (int a, int b, int c) triple = Dummy.TripleOf(Dummy.Int32().Between(1, 2), Dummy.Int32().Between(3, 4), Dummy.Int32().Between(5, 6)).Generate();
    ...
    public void CollectionsAreReproducible() {
        HashSet<int> first  = Dummy.SetOf(Dummy.WithSeed(4242).Int32()).WithCount(6).Generate();
        HashSet<int> second = Dummy.SetOf(Dummy.WithSeed(4242).Int32()).WithCount(6).Generate();

        Check.That(second.OrderBy(value => value)).ContainsExactly(first.OrderBy(value => value));

        List<int> listOne = Dummy.ListOf(Dummy.WithSeed(7).Int32().Between(0, 99)).WithCount(5).Generate();
        List<int> listTwo = Dummy.ListOf(Dummy.WithSeed(7).Int32().Between(0, 99)).WithCount(5).Generate();
    ...
    public void CollectionsComposeThroughAsAndCombine() {
        IDummy<List<OrderReference>> references = Dummy.ListOf(Dummy.String().StartingWith("ORD-").WithLength(12).As(OrderReference.Create)).WithCount(3);
    ...
    public void ExhaustionOverAForeignElementGeneratorQualifiesTheHint() {
        // A foreign IDummy carries no IHasRandomSource, so the collection falls back to the ambient source for its count
        ...
        Check.ThatCode(() => Dummy.Reproducibly(2026, () => Dummy.SetOf(new ForeignPair()).WithCount(5).Generate(), _ => { }))
             .Throws<DummyGenerationException>()
             ...
             .And.Contains("Dummy.Reproducibly(2026")
    ...
    public void ExhaustionOverAnAsDerivedForeignGeneratorQualifiesTheHint() {
        // DerivedDummy (from As) implements IHasRandomSource but propagates a null source when its operand is foreign,
        ...
        IDummy<int> derivedOverForeign = new ForeignPair().As(value => value);

        Check.ThatCode(() => Dummy.SetOf(derivedOverForeign).WithCount(5).Generate())
             .Throws<DummyGenerationException>()
    ...
    [Fact(DisplayName = "A foreign ContainingAny generator is qualified at its own site, and a fixed source is named as Dummy.WithSeed rather than Dummy.Reproducibly.")]
    public void ExhaustionOverAForeignContainingAnyQualifiesAndNamesTheFixedSource() {
        // The collection's own elements come from a fixed Dummy.WithSeed(...) context (faithful), but the ContainingAny
        // draw is foreign. The twin exhaustion site must qualify the hint for that specific generator — and, because
        // the collection's source is fixed, name Dummy.WithSeed, never the inapplicable Dummy.Reproducibly.
        DummyContext seeded = Dummy.WithSeed(4242);

        Check.ThatCode(() => Dummy.SetOf(seeded.Int32()).Containing(0).Containing(1).ContainingAny(new ForeignPair()).Generate())
             .Throws<DummyGenerationException>()
    ...
    public void ExhaustionOverALibraryElementGeneratorKeepsTheFaithfulHint() {
        ...
        Check.ThatCode(() => Dummy.Reproducibly(1234, () => Dummy.SetOf(Dummy.Int32().Between(0, 999), modTen).WithCount(20).Generate(), _ => { }))
             .Throws<DummyGenerationException>()
             .WithProperty(caught => caught.Seed, 1234)
             .And.WhichMember(caught => caught.Message)
             .Contains("The arbitrary values were seeded with 1234")
             .And.Contains("Dummy.Reproducibly(1234")
    ...
    public void ExhaustionOverACombineMixingAForeignOperandQualifiesTheHint() {
        // Dummy.Combine keeps the library operand's non-null source (SourceOf(first) ?? SourceOf(second)), but the
        ...
        IDummy<int> mixed = Dummy.Combine(new ForeignPair(), Dummy.Int32(), (foreign, _) => foreign);

        Check.ThatCode(() => Dummy.Reproducibly(777, () => Dummy.SetOf(mixed).WithCount(5).Generate(), _ => { }))
             .Throws<DummyGenerationException>()
    ...
    private sealed class ForeignPair : IDummy<int> {

        private int _n;

        // Foreign on purpose: implements IDummy<int> but NOT IHasRandomSource, so it does not draw from the collection's
