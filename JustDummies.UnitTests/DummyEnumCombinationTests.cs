...
/// <summary>
///     The named cases of <see cref="DummyEnum{TEnum}.AllowingCombinations" />: the exact universe a given enum shape
...
public sealed class DummyEnumCombinationTests {
    ...
    public void FlagsEnumDrawsDeclaredMembersByDefault() {
        ...
        for (int i = 0; i < SampleCount; i++) { seen.Add(Dummy.Enum<Permissions>().Generate()); }
    ...
    public void CombinationsCoverTheWholeUniverse() {
        HashSet<Permissions> seen = [];
        for (int i = 0; i < SampleCount; i++) { seen.Add(Dummy.Enum<Permissions>().AllowingCombinations().Generate()); }
    ...
    public void CombinationsOmitZeroWhenItIsNotDeclared() {
        HashSet<Sides> seen = [];
        for (int i = 0; i < SampleCount; i++) { seen.Add(Dummy.Enum<Sides>().AllowingCombinations().Generate()); }
    ...
    public void DeclaredCompositeDoesNotWidenTheUniverse() {
        HashSet<Access> seen = [];
        for (int i = 0; i < SampleCount; i++) { seen.Add(Dummy.Enum<Access>().AllowingCombinations().Generate()); }
    ...
    public void CombinationsRequireAFlagsEnum() {
        Check.ThatCode(() => Dummy.Enum<OrderStatus>().AllowingCombinations())
    ...
    public void CombinationsAreIdempotent() {
        DummyEnum<Permissions> generator = Dummy.Enum<Permissions>().AllowingCombinations().AllowingCombinations();
    ...
    public void OneOfAcceptsACombinationWithoutTheOptIn() {
        DummyEnum<Permissions> generator = Dummy.Enum<Permissions>().OneOf(Permissions.Read | Permissions.Write, Permissions.Exec);
    ...
    public void OneOfAndTheOptInAgreeInEitherOrder() {
        ...
        Check.That(Dummy.Enum<Permissions>().AllowingCombinations().OneOf(combination).Generate()).IsEqualTo(combination);
        Check.That(Dummy.Enum<Permissions>().OneOf(combination).AllowingCombinations().Generate()).IsEqualTo(combination);
        Check.That(Dummy.Enum<Permissions>().OneOf(combination).Generate()).IsEqualTo(combination);
    }
    ...
    public void OneOfRefusesAValueTheTypeDoesNotDefine() {
        ...
        ArgumentException error = Assert.Throws<ArgumentException>(() => Dummy.Enum<Permissions>().OneOf((Permissions)8));
    ...
    public void OneOfRefusesZeroWhereItIsNotDeclared() {
        ArgumentException error = Assert.Throws<ArgumentException>(() => Dummy.Enum<Sides>().OneOf(default(Sides)));
    ...
    public void OneOfRefusesACompositeOnANonFlagsEnum() {
        ...
        ArgumentException error = Assert.Throws<ArgumentException>(() => Dummy.Enum<OrderStatus>().OneOf((OrderStatus)7));
    ...
    public void OneOfAcceptsACombinationBeyondTheCombinableCeiling() {
        ...
        DummyEnum<WideBits> generator = Dummy.Enum<WideBits>().OneOf(WideBits.B00 | WideBits.B20);
    ...
    public void ExclusionsCompareByEquality() {
        DummyEnum<Permissions> generator = Dummy.Enum<Permissions>().AllowingCombinations().Except(Permissions.Read);
    ...
    public void CombinationsWidenTheCardinalityHint() {
        ...
        HashSet<Permissions> eight = Dummy.SetOf(Dummy.Enum<Permissions>().AllowingCombinations()).WithCount(8).Generate();
        Check.That(eight.Count).IsEqualTo(8);

        Check.ThatCode(() => Dummy.SetOf(Dummy.Enum<Permissions>().AllowingCombinations()).WithCount(9).Generate())
             .Throws<ConflictingDummyConstraintException>()
             .WhichMember(conflict => conflict.Message).Contains("9");

        Check.ThatCode(() => Dummy.SetOf(Dummy.Enum<Permissions>()).WithCount(5).Generate())
             .Throws<ConflictingDummyConstraintException>()
    ...
    public void ExcludingTheWholeUniverseConflicts() {
        ...
        Check.ThatCode(() => Dummy.Enum<Permissions>().AllowingCombinations().Except(everything).Generate())
             .Throws<ConflictingDummyConstraintException>()
    ...
    public void AllowingCombinationsIsHonouredWhereverItWasDeclared() {
        ...
        // in the other.
        Check.That(Dummy.Enum<Sides>().Except(Sides.Left, Sides.Right).AllowingCombinations().Generate()).IsEqualTo(Sides.Left | Sides.Right);
        Check.That(Dummy.Enum<Sides>().AllowingCombinations().Except(Sides.Left, Sides.Right).Generate()).IsEqualTo(Sides.Left | Sides.Right);
    }
    ...
    public void ExcludingEveryDeclaredMemberWithoutCombinationsIsStillRefused() {
        ...
        Check.ThatCode(() => Dummy.Enum<Sides>().Except(Sides.Left, Sides.Right).Generate())
             .Throws<ConflictingDummyConstraintException>()
    ...
    public void TooManyMembersIsRefused() {
        Check.ThatCode(() => Dummy.Enum<WideBits>().AllowingCombinations())
             .Throws<ConflictingDummyConstraintException>()
    ...
    public void CombinationsReplayUnderASeed() {
        List<Permissions> Batch(int seed) {
            ...
            DummyEnum<Permissions> generator = context.Enum<Permissions>().AllowingCombinations();
