...
/// <summary>
///     Named-case coverage for <see cref="DummyGuid" /> that a property cannot express: the two identifier pins
...
public sealed class AnyGuidTests {
    ...
    public void UnconstrainedDrawIsPracticallyNeverEmpty() {
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.Guid().Generate()).IsNotEqualTo(Guid.Empty);
        }
    }
