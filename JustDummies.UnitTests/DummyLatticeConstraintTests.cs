...
public sealed class AnyLatticeConstraintTests {
    ...
    public void MultipleOfAlwaysDivisible() {
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.Int32().MultipleOf(100).Generate() % 100).IsEqualTo(0);
            Check.That(Dummy.Int64().Positive().MultipleOf(12L).Generate() % 12L).IsEqualTo(0L);
            Check.That(Dummy.Byte().MultipleOf(5).Generate()               % 5).IsEqualTo(0);
        }
    }
    ...
    public void MultipleOfHonoursRangeAndReachesBounds() {
        HashSet<int> seen = [];
        for (int i = 0; i < SampleCount; i++) {
            int value = Dummy.Int32().Between(0, 1000).MultipleOf(100).Generate();
    ...
    public void MultipleOfHandlesNegativeGrid() {
        for (int i = 0; i < SampleCount; i++) {
            int value = Dummy.Int32().Between(-100, -1).MultipleOf(10).Generate();
    ...
    public void MultipleOfComposesWithExcept() {
        HashSet<int> seen = [];
        for (int i = 0; i < SampleCount; i++) {
            int value = Dummy.Int32().Between(0, 30).MultipleOf(10).Except(10, 20).Generate();
            Check.That(value % 10).IsEqualTo(0);
    ...
    public void MultipleOfFiltersAllowList() {
        for (int i = 0; i < SampleCount; i++) {
            int value = Dummy.Int32().OneOf(5, 10, 15, 20).MultipleOf(10).Generate();
            Check.That(value == 10 || value == 20).IsTrue();
        }
    }
    ...
    public void MultipleOfArguments() {
        for (int i = 0; i < SampleCount; i++) { Dummy.Int32().MultipleOf(1).Generate(); } // no-op: any value

        Check.ThatCode(() => Dummy.Int32().MultipleOf(0)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.Int32().MultipleOf(-5)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.Byte().MultipleOf(0)).Throws<ArgumentOutOfRangeException>();
    }
    ...
    public void MultipleOfEmptyGridConflicts() {
        Check.ThatCode(() => Dummy.Int32().Between(1, 9).MultipleOf(10))
        ...
        Check.ThatCode(() => Dummy.Int32().MultipleOf(10).Between(1, 9)).Throws<ConflictingDummyConstraintException>();
    }

    [Fact(DisplayName = "MultipleOf: a second, different step is rejected as already declared.")]
    public void MultipleOfDeclaredOnce() {
        Check.ThatCode(() => Dummy.Int32().MultipleOf(4).MultipleOf(6)).Throws<ConflictingDummyConstraintException>();
        // The same step twice is idempotent, not a conflict.
        Check.That(Dummy.Int32().Between(0, 100).MultipleOf(10).MultipleOf(10).Generate() % 10).IsEqualTo(0);
    }

    [Fact(DisplayName = "MultipleOf: a distinct collection sees the grid cardinality.")]
    public void MultipleOfFeedsCardinality() {
        HashSet<int> seen = [];
        for (int i = 0; i < SampleCount; i++) { seen.Add(Dummy.Int32().Between(0, 20).MultipleOf(10).Generate()); }
    ...
    public void WithScaleStaysOnGrid() {
        for (int i = 0; i < SampleCount; i++) {
            decimal amount = Dummy.Decimal().Between(0m, 1000m).WithScale(2).Generate();
            Check.That(amount).IsEqualTo(Math.Round(amount, 2, MidpointRounding.ToEven));
            Check.That(amount >= 0m && amount <= 1000m).IsTrue();

            decimal whole = Dummy.Decimal().WithScale(0).Generate();
    ...
    public void WithScaleReachesBounds() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            decimal value = Dummy.Decimal().Between(0m, 1m).WithScale(1).Generate();
    ...
    public void WithScaleComposesWithExcept() {
        for (int i = 0; i < SampleCount; i++) {
            decimal value = Dummy.Decimal().Between(0m, 1m).WithScale(1).Except(0.5m).Generate();
            Check.That(value).IsEqualTo(Math.Round(value, 1, MidpointRounding.ToEven));
    ...
    public void WithScaleArguments() {
        Check.ThatCode(() => Dummy.Decimal().WithScale(-1)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.Decimal().WithScale(29)).Throws<ArgumentOutOfRangeException>();
        Dummy.Decimal().WithScale(0).Generate();
        Dummy.Decimal().WithScale(28).Generate();
    }

    [Fact(DisplayName = "WithScale: a range containing no grid point conflicts eagerly.")]
    public void WithScaleEmptyGridConflicts() {
        Check.ThatCode(() => Dummy.Decimal().Between(0.001m, 0.009m).WithScale(2))
             .Throws<ConflictingDummyConstraintException>()
    ...
    public void WithGranularityStaysOnGrid() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.DateTime().WithGranularity(TimeSpan.FromMinutes(15)).Generate().Ticks % quarterHour).IsEqualTo(0L);
            Check.That(Dummy.TimeSpan().WithGranularity(TimeSpan.FromSeconds(1)).Generate().Ticks       % oneSecond).IsEqualTo(0L);
            Check.That(Dummy.DateTimeOffset().WithGranularity(TimeSpan.FromSeconds(1)).Generate().UtcTicks % oneSecond).IsEqualTo(0L);
        }
    }
    ...
    public void WithGranularityHonoursRange() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            DateTime value = Dummy.DateTime().Between(start, end).WithGranularity(TimeSpan.FromDays(1)).Generate();
    ...
    public void WithGranularityArguments() {
        Check.ThatCode(() => Dummy.DateTime().WithGranularity(TimeSpan.Zero)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.TimeSpan().WithGranularity(TimeSpan.FromTicks(-1))).Throws<ArgumentOutOfRangeException>();
    }
    ...
    public void WithGranularityEmptyGridConflicts() {
        ...
        Check.ThatCode(() => Dummy.DateTime().Between(start, end).WithGranularity(TimeSpan.FromDays(1)))
             .Throws<ConflictingDummyConstraintException>();
    }
    ...
    public void MultipleOfWideIntegers() {
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.Int128().Positive().MultipleOf((Int128)1000).Generate() % 1000 == (Int128)0).IsTrue();
            Check.That(Dummy.UInt128().MultipleOf((UInt128)7).Generate()             % 7 == (UInt128)0).IsTrue();
        }

        Check.ThatCode(() => Dummy.Int128().Between((Int128)1, (Int128)9).MultipleOf((Int128)10)).Throws<ConflictingDummyConstraintException>();
    }
    ...
    public void WithGranularityTimeOnly() {
        long oneSecond = TimeSpan.FromSeconds(1).Ticks;
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.TimeOnly().WithGranularity(TimeSpan.FromSeconds(1)).Generate().Ticks % oneSecond).IsEqualTo(0L);
        }
    }
