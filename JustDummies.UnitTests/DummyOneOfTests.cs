...
public sealed class AnyOneOfTests {
    ...
    private static IEnumerable<T> Samples<T>(IDummy<T> generator) {
    ...
    public void DrawsOnlyTheSuppliedValues() {
        ...
        foreach (Percentage value in Samples(Dummy.OneOf(ten, twenty, thirty))) {
    ...
    public void ReachesEverySuppliedValue() {
        HashSet<int> seen = [.. Samples(Dummy.OneOf(1, 2, 3))];
    ...
    public void SingleValueIsPinned() {
        foreach (int value in Samples(Dummy.OneOf(42))) {
    ...
    public void VariesAcrossDraws() {
        HashSet<int> seen = [.. Samples(Dummy.OneOf(1, 2, 3, 4))];
    ...
    public void DuplicatesAreCollapsed() {
        HashSet<int> seen = [.. Samples(Dummy.OneOf(1, 1, 2))];
    ...
    public void ReproducibleUnderASeed() {
        string first  = string.Join("|", Enumerable.Range(0, 20).Select(_ => Dummy.WithSeed(7).OneOf("a", "b", "c", "d").Generate()));
        string second = string.Join("|", Enumerable.Range(0, 20).Select(_ => Dummy.WithSeed(7).OneOf("a", "b", "c", "d").Generate()));
    ...
    public void ComposesThroughAs() {
        IDummy<OrderReference> generator = Dummy.OneOf("ORD-12345678", "ORD-87654321").As(OrderReference.Create);
    ...
    public void OrNullIsSometimesNull() {
        ...
        IDummy<Percentage?> generator = Dummy.WithSeed(20260721).OneOf(one, two).OrNull();
    ...
    public void CardinalityGatesDistinctCollections() {
        ...
        Check.ThatCode(() => Dummy.SetOf(Dummy.OneOf(1, 2)).WithCount(3).Generate()).Throws<ConflictingDummyConstraintException>();

        // Within the domain it fills the set with the requested distinct values.
        HashSet<int> set = Dummy.SetOf(Dummy.OneOf(1, 2, 3)).WithCount(3).Generate();
    ...
    public void ReferenceIdentityKeepsDistinctInstancesDistinct() {
        ...
        HashSet<Percentage> set = Dummy.SetOf(Dummy.OneOf(first, second)).WithCount(2).Generate();
    ...
    public void RejectsInvalidPools() {
        Check.ThatCode(() => Dummy.OneOf<int>()).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.OneOf((string[])null!)).Throws<ArgumentNullException>();
        Check.ThatCode(() => Dummy.OneOf("a", null!)).Throws<ArgumentException>();
    }

    [Fact(DisplayName = "The null-element message points the caller at OrNull().")]
    public void NullElementMessagePointsAtOrNull() {
        ArgumentException error = Assert.Throws<ArgumentException>(() => Dummy.OneOf("a", null!));
    ...
    public void ElementOfDrawsFromTheList() {
        ...
        HashSet<int> seen = [.. Samples(Dummy.ElementOf(pool))];
    ...
    public void ElementOfMaterializesTheSequenceOnce() {
        ...
        DummyOneOf<int> generator = Dummy.ElementOf(Source());
        for (int i = 0; i < SampleCount; i++) {
            generator.Generate();
        }
    ...
    public void ElementOfValidatesItsPool() {
        Check.ThatCode(() => Dummy.ElementOf((IReadOnlyList<int>)null!)).Throws<ArgumentNullException>();
        Check.ThatCode(() => Dummy.ElementOf((IEnumerable<int>)null!)).Throws<ArgumentNullException>();
        Check.ThatCode(() => Dummy.ElementOf(new List<int>())).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.ElementOf(Enumerable.Empty<int>())).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.ElementOf(["a", null!])).Throws<ArgumentException>();
    }
    ...
    public void DifferentFromRemovesTheValue() {
        ...
        foreach (Percentage value in Samples(Dummy.ElementOf(orders).DifferentFrom(used))) {
    ...
    public void ExceptRemovesEveryValue() {
        foreach (int value in Samples(Dummy.OneOf(1, 2, 3, 4).Except(2, 3))) {
            Check.That(new[] { 1, 4 }).Contains(value);
        }

        foreach (int value in Samples(Dummy.OneOf(1, 2, 3, 4).Except(2).DifferentFrom(3).Except(4))) {
    ...
    public void ExcludingAnAbsentValueRemovesNothing() {
        HashSet<int> seen = [.. Samples(Dummy.OneOf(1, 2).DifferentFrom(99))];
    ...
    public void AHeldCollectionPassedToOneOfIsOnePoolMember() {
        ...
        IReadOnlyList<int> drawn = Dummy.OneOf(held).Generate();
        ...
        Check.That(held.Contains(Dummy.ElementOf(held).Generate())).IsTrue();
    }
    ...
    public void AnExclusionEmptyingThePoolConflicts() {
        Check.ThatCode(() => Dummy.OneOf(1, 2).Except(1, 2))
    ...
    public void TheConflictNamesTheDeclaringFactory() {
        ...
        static string Emptied(Func<object> declare) {
            return Assert.Throws<ConflictingDummyConstraintException>(() => declare()).Message;
        }
        ...
        Check.That(Emptied(() => Dummy.OneOf(7).DifferentFrom(7))).IsEqualTo(fromOneOf);
        Check.That(Emptied(() => Dummy.WithSeed(1).OneOf(7).DifferentFrom(7))).IsEqualTo(fromOneOf);

        Check.That(Emptied(() => Dummy.ElementOf([7]).DifferentFrom(7))).IsEqualTo(fromElement);
        Check.That(Emptied(() => Dummy.ElementOf(new List<int> { 7 }.Select(value => value)).DifferentFrom(7))).IsEqualTo(fromElement);
        Check.That(Emptied(() => Dummy.WithSeed(1).ElementOf([7]).DifferentFrom(7))).IsEqualTo(fromElement);
        Check.That(Emptied(() => Dummy.WithSeed(1).ElementOf(new List<int> { 7 }.Select(value => value)).DifferentFrom(7))).IsEqualTo(fromElement);
    }
    ...
    public void AnExclusionLeavingADeclaredValueQualifiesItsClaim() {
        ...
        Check.ThatCode(() => Dummy.OneOf(1, 2).DifferentFrom(1).DifferentFrom(2))
             .Throws<ConflictingDummyConstraintException>()
    ...
    public void AnExclusionCoveringTheWholePoolIsNotQualifiedAway() {
        ...
        ConflictingDummyConstraintException afterAnother = Assert.Throws<ConflictingDummyConstraintException>(
            () => Dummy.OneOf(1, 2).DifferentFrom(1).Except(1, 2));
        ConflictingDummyConstraintException onItsOwn = Assert.Throws<ConflictingDummyConstraintException>(
            () => Dummy.OneOf(1, 2).Except(1, 2));
    ...
    public void CardinalityFollowsTheFilteredPool() {
        ...
        // the survivors.
        Check.ThatCode(() => Dummy.SetOf(Dummy.OneOf(1, 2, 3).DifferentFrom(2)).WithCount(3).Generate()).Throws<ConflictingDummyConstraintException>();

        HashSet<int> set = Dummy.SetOf(Dummy.OneOf(1, 2, 3).DifferentFrom(2)).WithCount(2).Generate();
    ...
    public void ExclusionArgumentsAreValidated() {
        Check.ThatCode(() => Dummy.OneOf("a", "b").Except(null!)).Throws<ArgumentNullException>();
        Check.ThatCode(() => Dummy.OneOf("a", "b").Except()).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.OneOf("a", "b").Except("a", null!)).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.OneOf("a", "b").DifferentFrom(null!)).Throws<ArgumentNullException>();
    }
    ...
    public void SeededContextIsDeterministic() {
        ...
        string oneOfFirst  = string.Join("|", Samples(Dummy.WithSeed(11).OneOf(10, 20, 30, 40)).Take(20));
        string oneOfSecond = string.Join("|", Samples(Dummy.WithSeed(11).OneOf(10, 20, 30, 40)).Take(20));
        Check.That(oneOfSecond).IsEqualTo(oneOfFirst);

        string elementFirst  = string.Join("|", Samples(Dummy.WithSeed(11).ElementOf(pool)).Take(20));
        string elementSecond = string.Join("|", Samples(Dummy.WithSeed(11).ElementOf(pool)).Take(20));
