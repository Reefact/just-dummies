...
public sealed class AnyPatternTests {
    ...
    public void GeneratedValuesMatchTheRealEngine(string pattern) {
        DummyContext context   = Dummy.WithSeed(20260718);
        DummyPattern generator = context.StringMatching(pattern);
    ...
    public void GeneratedValuesVary() {
        foreach (string pattern in new[] { @"\d{8}", @"[A-Z]{3}", @"(EUR|USD|GBP)", @"[A-Za-z0-9_]+", @"a{2,4}b*c+" }) {
            DummyPattern      generator = Dummy.WithSeed(4242).StringMatching(pattern);
            HashSet<string> seen      = [];
    ...
    public void FixedShape() {
        for (int i = 0; i < SampleCount; i++) {
            string reference = Dummy.StringMatching(@"^ORD-\d{8}$").Generate();
    ...
    public void Alternation() {
        HashSet<string> seen = [];
        for (int i = 0; i < SampleCount; i++) {
            string value = Dummy.StringMatching("(EUR|USD|GBP)").Generate();
    ...
    public void CharacterClasses() {
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.StringMatching("[A-Z]{3}").Generate()).Matches("^[A-Z]{3}$");
            Check.That(Dummy.StringMatching("[^0-9]{4}").Generate()).Matches("^[^0-9]{4}$");
            Check.That(Dummy.StringMatching(@"[\d]{5}").Generate()).Matches("^[0-9]{5}$");
        }
    }
    ...
    public void QuantifierBounds() {
        ...
        for (int i = 0; i < SampleCount; i++) {
            int bounded = (Dummy.StringMatching("a{2,4}").Generate()).Length;
            Check.That(bounded is >= 2 and <= 4).IsTrue();

            starLengths.Add((Dummy.StringMatching("a*").Generate()).Length);
            plusLengths.Add((Dummy.StringMatching("a+").Generate()).Length);
            openLengths.Add((Dummy.StringMatching("a{2,}").Generate()).Length);
        }
    ...
    public void AnchorsAreNoOps() {
        for (int i = 0; i < SampleCount; i++) {
            Check.That(Dummy.StringMatching("^abc$").Generate()).IsEqualTo("abc");
        }
    }
    ...
    public void RepeatedAndQuantifiedAnchorsAreNoOps() {
        ...
        Check.That(Dummy.StringMatching(@"^^abc$$").Generate()).IsEqualTo("abc");
        Check.That(Dummy.StringMatching(@"^*abc$*").Generate()).IsEqualTo("abc");
        Check.That(Dummy.StringMatching(@"^{2}abc").Generate()).IsEqualTo("abc");
        Check.That(Dummy.StringMatching(@"^?abc$?").Generate()).IsEqualTo("abc");
    }
    ...
    public void UnboundedQuantifierAtTheTopOfTheIntRangeNeverYieldsAShortValue() {
        ...
        DummyPattern generator = Dummy.StringMatching("a{2147483647,}");
    ...
    public void EveryMinimumInTheOverflowBandOverrunsTheCeiling(int minimum) {
        ...
        DummyPattern generator = Dummy.WithSeed(1).StringMatching($"a{{{minimum},}}");

        for (int i = 0; i < 12; i++) {
            Check.ThatCode(() => generator.Generate()).Throws<DummyGenerationException>();
        }
    }
    ...
    public void OverLimitPatternFailsWithoutAFalseCause() {
        ...
        DummyPattern generator = Dummy.StringMatching(@"(a{1000}){1000}");
    ...
    public void NegatedClassExcludingTheUniverseIsUnsupported() {
        ...
        Check.ThatCode(() => Dummy.StringMatching(@"[^\x20-\x7E]")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"[^\s\S]")).Throws<UnsupportedRegexException>();

        Check.ThatCode(() => Dummy.StringMatching(@"[^\x20-\x7E]"))
    ...
    public void PatternsAcceptedByTheRealEngineAreNeverMalformed(string pattern) {
        ...
        try {
            Dummy.WithSeed(1).StringMatching(pattern).Generate();
    ...
    public void IgnoreCaseHonoured() {
        ...
        bool            sawUpper = false;
        DummyContext      context  = Dummy.WithSeed(99);
        DummyPattern      generator = context.StringMatching(pattern);
    ...
    public void ComposesThroughAs() {
        IDummy<OrderReference> generator = Dummy.StringMatching(@"^ORD-\d{8}$").As(OrderReference.Create);
    ...
    public void ReproducibleUnderASeed() {
        string first  = string.Join("|", Enumerable.Range(0, 20).Select(_ => Dummy.WithSeed(7).StringMatching(@"[A-Z]{3}-\d{4}").Generate()));
        string second = string.Join("|", Enumerable.Range(0, 20).Select(_ => Dummy.WithSeed(7).StringMatching(@"[A-Z]{3}-\d{4}").Generate()));
    ...
    public void DifferentFromNeverYieldsTheExcludedValue() {
        ...
        IDummy<string> generator = Dummy.WithSeed(20260728).StringMatching(pattern).DifferentFrom(existing);
    ...
    public void ExceptRemovesEverySuppliedValue() {
        ...
        // removed each word.
        IDummy<string> generator = Dummy.WithSeed(20260728).StringMatching("^[ab]{2}$").Except("ab", "ba").DifferentFrom("bb");
    ...
    public void AnExclusionStaysReproducibleUnderASeed() {
        string first  = string.Join("|", Enumerable.Range(0, 20).Select(_ => Dummy.WithSeed(7).StringMatching(@"[A-Z]{3}-\d{4}").DifferentFrom("ABC-1234").Generate()));
        string second = string.Join("|", Enumerable.Range(0, 20).Select(_ => Dummy.WithSeed(7).StringMatching(@"[A-Z]{3}-\d{4}").DifferentFrom("ABC-1234").Generate()));
    ...
    public void AnExhaustedExclusionBudgetReportsTheBudget() {
        ...
        Check.ThatCode(() => Dummy.WithSeed(20260728).StringMatching("^[ab]$").Except("a", "b").Generate())
             .Throws<DummyGenerationException>()
    ...
    public void TheExhaustionCarriesTheSeed() {
        Check.ThatCode(() => Dummy.WithSeed(20260728).StringMatching("^[ab]$").Except("a", "b").Generate())
             ...
             .And.WhichMember(error => error.Message).Contains("\"^[ab]$\"", "\"a\", \"b\"", "Dummy.WithSeed(20260728)");
    }
    ...
    public void RepeatedExclusionsCollapse() {
        Check.ThatCode(() => Dummy.WithSeed(20260728).StringMatching("^[ab]$").Except("a").DifferentFrom("a").Except("a", "b").Generate())
             .Throws<DummyGenerationException>()
             .WhichMember(error => error.Message)
    ...
    public void OnlyTheRejectivePairIsOffered() {
        ...
        string[] fluent = typeof(DummyPattern)
                          .GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly)
                          .Where(method => method.ReturnType == typeof(DummyPattern) && !method.IsSpecialName)
    ...
    public void ExclusionArgumentsAreValidated() {
        Check.ThatCode(() => Dummy.StringMatching("a").Except(null!)).Throws<ArgumentNullException>();
        Check.ThatCode(() => Dummy.StringMatching("a").Except()).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.StringMatching("a").Except("a", null!)).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.StringMatching("a").DifferentFrom(null!)).Throws<ArgumentNullException>();
    }
    ...
    public void UnsupportedConstructsAreRefused() {
        Check.ThatCode(() => Dummy.StringMatching(@"foo(?=bar)")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"foo(?!bar)")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"(?<=x)y")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"\bword\b")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"(\w+)\s\1")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"\p{L}+")).Throws<UnsupportedRegexException>();

        Check.ThatCode(() => Dummy.StringMatching(@"a(?=b)"))
             .Throws<UnsupportedRegexException>()
    ...
    public void NotGeneratableConstructsAreRefused() {
        ...
        Check.ThatCode(() => Dummy.StringMatching(@"(?>ab|a)b")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"(?>a)")).Throws<UnsupportedRegexException>();

        // A misplaced anchor makes the pattern unmatchable by any whole string.
        Check.ThatCode(() => Dummy.StringMatching(@"a^")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"$a")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"x(^a)")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"(a$)x")).Throws<UnsupportedRegexException>();
        ...
        Check.ThatCode(() => Dummy.StringMatching(@"[a-z-[aeiou]]")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"[a-[x]]")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"[ab-[b]]")).Throws<UnsupportedRegexException>();

        // IgnorePatternWhitespace changes how the pattern text itself is read: "^A B$" matches "AB", not "A B".
        Check.ThatCode(() => Dummy.StringMatching(new Regex("^A B$", RegexOptions.IgnorePatternWhitespace))).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.WithSeed(1).StringMatching(new Regex("^A B$", RegexOptions.IgnorePatternWhitespace))).Throws<ArgumentException>();
    }
    ...
    public void BalancingGroupsAreRefused() {
        ...
        Check.ThatCode(() => Dummy.StringMatching(@"(?<a>y)?(?<-a>x)")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"(?<a>y)?(?'-a'x)")).Throws<UnsupportedRegexException>(); // quote form
        ...
        Check.ThatCode(() => Dummy.StringMatching(@"(?<-a>x)")).Throws<UnsupportedRegexException>();
        Check.ThatCode(() => Dummy.StringMatching(@"(?'-a'x)")).Throws<UnsupportedRegexException>();          // quote form
        Check.ThatCode(() => Dummy.StringMatching(@"(?<x-y>z)")).Throws<UnsupportedRegexException>();         // name1-name2 form

        Check.ThatCode(() => Dummy.StringMatching(@"(?<a>y)?(?<-a>x)"))
             .Throws<UnsupportedRegexException>()
    ...
    public void MalformedPatternsAreRejected() {
        Check.ThatCode(() => Dummy.StringMatching(@"[a-")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.StringMatching(@"(abc")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.StringMatching(@"a{3,1}")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.StringMatching(@"*abc")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.StringMatching(@"a\")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.StringMatching((string)null!)).Throws<ArgumentNullException>();
        Check.ThatCode(() => Dummy.StringMatching((Regex)null!)).Throws<ArgumentNullException>();
    }
    ...
    public void RealEngineRejectionsAreMirrored() {
        ...
        Check.ThatCode(() => Dummy.StringMatching(@"a*+")).Throws<ArgumentException>();      // possessive: not a .NET construct
        Check.ThatCode(() => Dummy.StringMatching(@"a**")).Throws<ArgumentException>();      // nested quantifier
        Check.ThatCode(() => Dummy.StringMatching(@"a*??")).Throws<ArgumentException>();     // nested quantifier
        Check.ThatCode(() => Dummy.StringMatching(@"[]")).Throws<ArgumentException>();       // unterminated class
        Check.ThatCode(() => Dummy.StringMatching(@"\q")).Throws<ArgumentException>();       // unrecognized escape
        Check.ThatCode(() => Dummy.StringMatching(@"\x4")).Throws<ArgumentException>();      // \x expects 2 hex digits
        Check.ThatCode(() => Dummy.StringMatching(@"\c1")).Throws<ArgumentException>();      // \c expects a letter
        Check.ThatCode(() => Dummy.StringMatching(@"{2}")).Throws<ArgumentException>();      // quantifier following nothing
        Check.ThatCode(() => Dummy.StringMatching(@"(?<>a)")).Throws<ArgumentException>();   // empty group name
    }
    ...
    public void InvalidGroupNamesAreRejected() {
        ...
        Check.ThatCode(() => Dummy.StringMatching(@"(?<1a>x)")).Throws<ArgumentException>();  // digit then letter
        Check.ThatCode(() => Dummy.StringMatching(@"(?<0>x)")).Throws<ArgumentException>();   // group 0 is reserved
        Check.ThatCode(() => Dummy.StringMatching(@"(?<01>x)")).Throws<ArgumentException>();  // leading zero
        Check.ThatCode(() => Dummy.StringMatching(@"(?'0'x)")).Throws<ArgumentException>();   // quote form, reserved

        // A non-numeric name must be word characters (letter, digit or underscore); a space or a dot is malformed.
        Check.ThatCode(() => Dummy.StringMatching(@"(?<a b>x)")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.StringMatching(@"(?'a b'x)")).Throws<ArgumentException>(); // quote form
        Check.ThatCode(() => Dummy.StringMatching(@"(?<a.b>x)")).Throws<ArgumentException>();
    }
    ...
    public void EscapesGenerateTheRealCharacters() {
        Check.That(Dummy.StringMatching(@"\a").Generate()).IsEqualTo("\a");
        Check.That(Dummy.StringMatching(@"\e").Generate()).IsEqualTo("\u001B");
        Check.That(Dummy.StringMatching(@"\x41").Generate()).IsEqualTo("A");
        Check.That(Dummy.StringMatching(@"\u0042").Generate()).IsEqualTo("B");
        Check.That(Dummy.StringMatching(@"\cA").Generate()).IsEqualTo("\u0001");
        Check.That(Dummy.StringMatching(@"\07").Generate()).IsEqualTo("\a");
        Check.That(Dummy.StringMatching(@"\0").Generate()).IsEqualTo("\0");
    }
    ...
    public void BraceLiteralsGenerate() {
        Check.That(Dummy.StringMatching(@"a{x}").Generate()).IsEqualTo("a{x}");
        Check.That(Dummy.StringMatching(@"{abc}").Generate()).IsEqualTo("{abc}");
        Check.That(Dummy.StringMatching(@"a{2,").Generate()).IsEqualTo("a{2,");
    }
    ...
    public void DeepNestingFailsCleanly() {
        ...
        ArgumentException caught = Assert.Throws<ArgumentException>(() => Dummy.StringMatching(deep));
    ...
    public async Task ClassRangeEndingAtMaxCharTerminates(string pattern) {
        ...
        // fails the test instead of hanging the whole suite (mirrors the DummyGuid carry-wraparound guard).
        Task<string> run   = Task.Run(() => Dummy.StringMatching(pattern).Generate());
    ...
    public void ANullableAlternativeUnderAQuantifierNeverYieldsAnUnmatchedValue() {
        ...
        IDummy<string> generator = Dummy.StringMatching(pattern);
