...
public sealed class AnyUriTests {
    ...
    private static void GeneratesAcross(Func<DummyContext, IDummy<Uri>> build, UriKind? kind) {
        for (int seed = 0; seed < FuzzSeeds; seed++) {
            Uri value;
            try { value = build(Dummy.WithSeed(seed)).Generate(); }
    ...
    private static IDummy<Uri> Seeded(Func<DummyContext, IDummy<Uri>> build) {
        return build(Dummy.WithSeed(20260723));
    }
    ...
    public void SecondSchemePinConflicts() {
        Check.ThatCode(() => Dummy.Uri().Web().UsingHttp().UsingHttps())
    ...
    public void SecondPathConstraintConflicts() {
        Check.ThatCode(() => Dummy.Uri().Web().WithoutPath().WithPathSegments(2))
             .Throws<ConflictingDummyConstraintException>()
    ...
    public void WithHostRejectsIdnHost() {
        ArgumentException error = Assert.Throws<ArgumentException>(() => Dummy.Uri().Web().WithHost("münchen.de"));
    ...
    public void ArgumentsAreValidated() {
        Check.ThatCode(() => Dummy.Uri().Web().WithHost(null!)).Throws<ArgumentNullException>();
        Check.ThatCode(() => Dummy.Uri().Web().WithHost("")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.Uri().Web().WithHost("bad host")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.Uri().Web().WithUserInfo("a:b")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.Uri().Web().WithPort(0)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.Uri().Web().WithPort(70000)).Throws<ArgumentOutOfRangeException>();
        Check.ThatCode(() => Dummy.Uri().Web().WithPathSegments(-1)).Throws<ArgumentOutOfRangeException>();
    }
    ...
    public void EmptyRelativeFailsAtGeneration() {
        Check.ThatCode(() => Dummy.WithSeed(20260723).Uri().Relative().WithPathSegments(0).Generate())
    ...
    public void ZeroSegmentRelativeWithQueryGenerates() {
        Uri value = Dummy.WithSeed(1).Uri().Relative().WithPathSegments(0).WithQuery().Generate();
    ...
    public void WithHostRejectsNonCanonicalHost() {
        ArgumentException error = Assert.Throws<ArgumentException>(() => Dummy.Uri().Web().WithHost("123"));
        Check.That(error.Message).Contains("canonical");

        Check.ThatCode(() => Dummy.Uri().Web().WithHost("1.2")).Throws<ArgumentException>();
        Check.ThatCode(() => Dummy.Uri().Web().WithHost("1.2.3.4")).DoesNotThrow();
        Check.ThatCode(() => Dummy.Uri().Web().WithHost("api.example.com")).DoesNotThrow();
    }
    ...
    public void SeededDrawIsReproducible() {
        Uri first  = Dummy.WithSeed(4242).Uri().Web().WithQuery().Generate();
        Uri second = Dummy.WithSeed(4242).Uri().Web().WithQuery().Generate();
    ...
    private static IEnumerable<T> Sample<T>(IDummy<T> generator) {
