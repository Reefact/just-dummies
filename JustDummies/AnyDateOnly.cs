...
public sealed class AnyDateOnly : IAny<DateOnly>, IHasRandomSource, ICardinalityHint<DateOnly>, IPoolInspection<DateOnly> {
    ...
    /// <summary>
    ///     Requires the date to be one of the supplied values — the <see cref="IEnumerable{T}" /> counterpart of
    ///     <see cref="OneOf(DateOnly[])" />, for a set already held as a sequence (a list, a LINQ result, values loaded at
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty.</exception>
    /// <exception cref="ConflictingAnyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public AnyDateOnly OneOf(IEnumerable<DateOnly> values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return OneOf(values as DateOnly[] ?? values.ToArray());
    }
