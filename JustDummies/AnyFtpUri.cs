...
public sealed class AnyFtpUri : IAny<Uri>, IHasRandomSource {
    ...
    public AnyFtpUri WithUserInfo() {
        return new AnyFtpUri(_source, _spec.WithUserInfo(null, null, UriSpec.Label(nameof(WithUserInfo))));
    }
    ...
    public AnyFtpUri WithUserInfo(string user) {
        return new AnyFtpUri(_source, _spec.WithUserInfo(UriSpec.RequireUserInfoPart(user, nameof(user)), null, UriSpec.Label(nameof(WithUserInfo), user)));
    }
    ...
    public AnyFtpUri WithUserInfo(string user, string password) {
        return new AnyFtpUri(_source, _spec.WithUserInfo(UriSpec.RequireUserInfoPart(user, nameof(user)), UriSpec.RequireUserInfoPart(password, nameof(password)), UriSpec.Label(nameof(WithUserInfo), user, password)));
    }

    /// <summary>Includes an arbitrary non-default port.</summary>
    public AnyFtpUri WithPort() {
        return new AnyFtpUri(_source, _spec.WithPort(null, UriSpec.Label(nameof(WithPort))));
    }
    ...
    public AnyFtpUri WithPort(int port) {
        return new AnyFtpUri(_source, _spec.WithPort(UriSpec.RequirePort(port, nameof(port)), UriSpec.Label(nameof(WithPort), port)));
    }
    ...
    public AnyFtpUri WithPathSegments(int count) {
        return new AnyFtpUri(_source, _spec.WithPath(UriPathMode.Exact, UriSpec.RequireSegmentCount(count, nameof(count)), UriSpec.Label(nameof(WithPathSegments), count)));
    }

    /// <summary>Renders the root path (<c>/</c>) with no segments. Declared once per generator.</summary>
    /// <exception cref="ConflictingAnyConstraintException">Thrown when a path constraint is already declared.</exception>
    public AnyFtpUri WithoutPath() {
        return new AnyFtpUri(_source, _spec.WithPath(UriPathMode.Root, 0, ConstraintCall.Of(nameof(WithoutPath))));
    }
