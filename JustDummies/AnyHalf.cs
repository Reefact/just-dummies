...
///     </para>
///     <para>
///         <c>Except(Half.NaN)</c> and <c>DifferentFrom(Half.NaN)</c> are accepted instead, as a no-op: a non-finite
///         value is already guaranteed never to be drawn, so excluding one asks for nothing the generator does not
///         already give. This matters for <c>DifferentFrom</c> in particular, routinely written over a value a test
///         already holds — refusing it would turn a harmless exclusion into a failure whenever that value happened
///         to be infinite.
///     </para>
///     <para>
...
public sealed class AnyHalf : IAny<Half>, IHasRandomSource, ICardinalityHint<Half>, IPoolInspection<Half> {
    ...
    ///     <see cref="OneOf(Half[])" />, for a set already held as a sequence (a list, a LINQ result, values loaded at test
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty or contains a non-finite value.</exception>
    /// <exception cref="ConflictingAnyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public AnyHalf OneOf(IEnumerable<Half> values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return OneOf(values as Half[] ?? values.ToArray());
    }
