...
public sealed class AnyMailtoUri : IAny<Uri>, IHasRandomSource {
    ...
    public AnyMailtoUri WithLocalPart(string localPart) {
        return new AnyMailtoUri(_source, _spec.WithUserInfo(UriSpec.RequireUserInfoPart(localPart, nameof(localPart)), null, UriSpec.Label(nameof(WithLocalPart), localPart)));
    }
