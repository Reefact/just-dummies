...
/// </summary>
public sealed class AnySByte : IAny<sbyte>, IHasRandomSource, ICardinalityHint<sbyte>, IPoolInspection<sbyte> {
    ...
    public AnySByte Positive() {
        return new AnySByte(_source, _spec.WithMinimum(Ord(1), ConstraintCall.Of(nameof(Positive))));
    }
    ...
    public AnySByte Negative() {
        return new AnySByte(_source, _spec.WithMaximum(Ord(-1), ConstraintCall.Of(nameof(Negative))));
    }
    ...
    public AnySByte Zero() {
        return new AnySByte(_source, _spec.WithMinimum(Ord(0), ConstraintCall.Of(nameof(Zero))).WithMaximum(Ord(0), ConstraintCall.Of(nameof(Zero))));
    }
    ...
    public AnySByte NonZero() {
        return new AnySByte(_source, _spec.WithExcluded([Ord(0)], ConstraintCall.Of(nameof(NonZero))));
    }
    ...
    public AnySByte GreaterThan(sbyte value) {
        return new AnySByte(_source, _spec.WithMinimumAbove(Ord(value), ConstraintCall.Of(nameof(GreaterThan), V(value))));
    }
    ...
    public AnySByte GreaterThanOrEqualTo(sbyte value) {
        return new AnySByte(_source, _spec.WithMinimum(Ord(value), ConstraintCall.Of(nameof(GreaterThanOrEqualTo), V(value))));
    }
    ...
    public AnySByte LessThan(sbyte value) {
        return new AnySByte(_source, _spec.WithMaximumBelow(Ord(value), ConstraintCall.Of(nameof(LessThan), V(value))));
    }
    ...
    public AnySByte LessThanOrEqualTo(sbyte value) {
        return new AnySByte(_source, _spec.WithMaximum(Ord(value), ConstraintCall.Of(nameof(LessThanOrEqualTo), V(value))));
    }
    ...
    /// <exception cref="ConflictingAnyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public AnySByte MultipleOf(sbyte value) {
        if (value <= 0) { throw new ArgumentOutOfRangeException(nameof(value), value, "The multiple must be strictly positive."); }

        return new AnySByte(_source, _spec.WithStep((ulong)value, Ord(0), ConstraintCall.Of(nameof(MultipleOf), V(value))));
    }
    ...
    public AnySByte OneOf(params sbyte[] values) {
        ...
        return new AnySByte(_source, _spec.WithAllowed(values.Select(Ord).ToArray(), ConstraintCall.Of(nameof(OneOf), Join(values))));
    }

    /// <summary>
    ///     Requires the value to be one of the supplied values — the <see cref="IEnumerable{T}" /> counterpart of
    ///     <see cref="OneOf(sbyte[])" />, for a set already held as a sequence (a list, a LINQ result, values loaded at test
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty.</exception>
    /// <exception cref="ConflictingAnyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public AnySByte OneOf(IEnumerable<sbyte> values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return OneOf(values as sbyte[] ?? values.ToArray());
    }
    ...
    public AnySByte Except(params sbyte[] values) {
        ...
        return new AnySByte(_source, _spec.WithExcluded(values.Select(Ord).ToArray(), ConstraintCall.Of(nameof(Except), Join(values))));
    }
    ...
    public AnySByte DifferentFrom(sbyte value) {
        return new AnySByte(_source, _spec.WithExcluded([Ord(value)], ConstraintCall.Of(nameof(DifferentFrom), V(value))));
    }
