...
///     </para>
///     <para>
///         <c>Except(float.NaN)</c> and <c>DifferentFrom(float.NaN)</c> are accepted instead, as a no-op: a
...
public sealed class AnySingle : IAny<float>, IHasRandomSource, ICardinalityHint<float>, IPoolInspection<float> {
    ...
    private const string TypeName = "float";
    ...
    public AnySingle OneOf(params float[] values) {
        ...
        foreach (float value in values) { ContinuousIntervalSpec.EnsureFiniteForPool(value, nameof(values), TypeName); }
    ...
    ///     <see cref="OneOf(float[])" />, for a set already held as a sequence (a list, a LINQ result, values loaded at test
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty or contains a non-finite value.</exception>
    /// <exception cref="ConflictingAnyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public AnySingle OneOf(IEnumerable<float> values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return OneOf(values as float[] ?? values.ToArray());
    }
