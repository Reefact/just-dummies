...
public sealed class AnyTimeSpan : IAny<TimeSpan>, IHasRandomSource, ICardinalityHint<TimeSpan>, IPoolInspection<TimeSpan> {
    ...
    /// <summary>
    ///     Requires the duration to be one of the supplied values — the <see cref="IEnumerable{T}" /> counterpart of
    ///     <see cref="OneOf(TimeSpan[])" />, for a set already held as a sequence (a list, a LINQ result, values loaded at
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty.</exception>
    /// <exception cref="ConflictingAnyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public AnyTimeSpan OneOf(IEnumerable<TimeSpan> values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return OneOf(values as TimeSpan[] ?? values.ToArray());
    }
