...
/// </summary>
public sealed class AnyUInt16 : IAny<ushort>, IHasRandomSource, ICardinalityHint<ushort>, IPoolInspection<ushort> {
    ...
    public AnyUInt16 Zero() {
        return new AnyUInt16(_source, _spec.WithMinimum(Ord(0), ConstraintCall.Of(nameof(Zero))).WithMaximum(Ord(0), ConstraintCall.Of(nameof(Zero))));
    }
    ...
    public AnyUInt16 NonZero() {
        return new AnyUInt16(_source, _spec.WithExcluded([Ord(0)], ConstraintCall.Of(nameof(NonZero))));
    }
    ...
    public AnyUInt16 GreaterThan(ushort value) {
        return new AnyUInt16(_source, _spec.WithMinimumAbove(Ord(value), ConstraintCall.Of(nameof(GreaterThan), V(value))));
    }
    ...
    public AnyUInt16 GreaterThanOrEqualTo(ushort value) {
        return new AnyUInt16(_source, _spec.WithMinimum(Ord(value), ConstraintCall.Of(nameof(GreaterThanOrEqualTo), V(value))));
    }
    ...
    public AnyUInt16 LessThan(ushort value) {
        return new AnyUInt16(_source, _spec.WithMaximumBelow(Ord(value), ConstraintCall.Of(nameof(LessThan), V(value))));
    }
    ...
    public AnyUInt16 LessThanOrEqualTo(ushort value) {
        return new AnyUInt16(_source, _spec.WithMaximum(Ord(value), ConstraintCall.Of(nameof(LessThanOrEqualTo), V(value))));
    }
    ...
    /// <exception cref="ConflictingAnyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public AnyUInt16 MultipleOf(ushort value) {
        if (value == 0) { throw new ArgumentOutOfRangeException(nameof(value), value, "The multiple must be strictly positive."); }

        return new AnyUInt16(_source, _spec.WithStep((ulong)value, Ord(0), ConstraintCall.Of(nameof(MultipleOf), V(value))));
    }
    ...
    public AnyUInt16 OneOf(params ushort[] values) {
        ...
        return new AnyUInt16(_source, _spec.WithAllowed(values.Select(Ord).ToArray(), ConstraintCall.Of(nameof(OneOf), Join(values))));
    }

    /// <summary>
    ///     Requires the value to be one of the supplied values — the <see cref="IEnumerable{T}" /> counterpart of
    ///     <see cref="OneOf(ushort[])" />, for a set already held as a sequence (a list, a LINQ result, values loaded at
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty.</exception>
    /// <exception cref="ConflictingAnyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public AnyUInt16 OneOf(IEnumerable<ushort> values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return OneOf(values as ushort[] ?? values.ToArray());
    }
    ...
    public AnyUInt16 Except(params ushort[] values) {
        ...
        return new AnyUInt16(_source, _spec.WithExcluded(values.Select(Ord).ToArray(), ConstraintCall.Of(nameof(Except), Join(values))));
    }
    ...
    public AnyUInt16 DifferentFrom(ushort value) {
        return new AnyUInt16(_source, _spec.WithExcluded([Ord(value)], ConstraintCall.Of(nameof(DifferentFrom), V(value))));
    }
