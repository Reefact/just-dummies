...
public sealed class AnyUInt64 : IAny<ulong>, IHasRandomSource, ICardinalityHint<ulong>, IPoolInspection<ulong> {
    ...
    ///     <see cref="OneOf(ulong[])" />, for a set already held as a sequence (a list, a LINQ result, values loaded at test
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty.</exception>
    /// <exception cref="ConflictingAnyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public AnyUInt64 OneOf(IEnumerable<ulong> values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return OneOf(values as ulong[] ?? values.ToArray());
    }
