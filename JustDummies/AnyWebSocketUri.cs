...
public sealed class AnyWebSocketUri : IAny<Uri>, IHasRandomSource {
    ...
    public AnyWebSocketUri UsingWs() {
        return new AnyWebSocketUri(_source, _spec.WithScheme("ws", ConstraintCall.Of(nameof(UsingWs))));
    }

    /// <summary>Pins the scheme to <c>wss</c>. Declared once per generator.</summary>
    public AnyWebSocketUri UsingWss() {
        return new AnyWebSocketUri(_source, _spec.WithScheme("wss", ConstraintCall.Of(nameof(UsingWss))));
    }
    ...
    public AnyWebSocketUri WithHost(string host) {
        return new AnyWebSocketUri(_source, _spec.WithHost(UriSpec.RequireHost(host, nameof(host)), UriSpec.Label(nameof(WithHost), host)));
    }

    /// <summary>Includes an arbitrary non-default port.</summary>
    public AnyWebSocketUri WithPort() {
        return new AnyWebSocketUri(_source, _spec.WithPort(null, UriSpec.Label(nameof(WithPort))));
    }
    ...
    public AnyWebSocketUri WithPort(int port) {
        return new AnyWebSocketUri(_source, _spec.WithPort(UriSpec.RequirePort(port, nameof(port)), UriSpec.Label(nameof(WithPort), port)));
    }
    ...
    public AnyWebSocketUri WithPathSegments(int count) {
        return new AnyWebSocketUri(_source, _spec.WithPath(UriPathMode.Exact, UriSpec.RequireSegmentCount(count, nameof(count)), UriSpec.Label(nameof(WithPathSegments), count)));
    }

    /// <summary>Renders the root path (<c>/</c>) with no segments. Declared once per generator.</summary>
    /// <exception cref="ConflictingAnyConstraintException">Thrown when a path constraint is already declared.</exception>
    public AnyWebSocketUri WithoutPath() {
        return new AnyWebSocketUri(_source, _spec.WithPath(UriPathMode.Root, 0, ConstraintCall.Of(nameof(WithoutPath))));
    }
