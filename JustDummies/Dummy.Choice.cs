...
public static partial class Dummy {
    ...
    public static DummyOneOf<T> OneOf<T>(params T[] values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return DummyOneOf<T>.FromPool(AmbientRandomSource.Instance, values, ConstraintCall.OfElided(nameof(OneOf)));
    }
    ...
    ///     the pool — <c>Dummy.ElementOf(orders).DifferentFrom(theOneAlreadyUsed)</c> is the idiom this overload exists
    ///     for.
    /// </summary>
    /// <param name="values">The pool the generated value is drawn from; duplicates are ignored.</param>
    /// <typeparam name="T">The type of the pooled values.</typeparam>
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="values" /> is <c>null</c>.</exception>
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty or contains a <c>null</c> element.</exception>
    public static DummyOneOf<T> ElementOf<T>(IReadOnlyList<T> values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return DummyOneOf<T>.FromPool(AmbientRandomSource.Instance, values, ConstraintCall.OfElided(nameof(ElementOf)));
    }
    ...
    /// <exception cref="ArgumentNullException">Thrown when <paramref name="values" /> is <c>null</c>.</exception>
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty or contains a <c>null</c> element.</exception>
    public static DummyOneOf<T> ElementOf<T>(IEnumerable<T> values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }

        return DummyOneOf<T>.FromPool(AmbientRandomSource.Instance, values as IReadOnlyList<T> ?? values.ToArray(), ConstraintCall.OfElided(nameof(ElementOf)));
    }
