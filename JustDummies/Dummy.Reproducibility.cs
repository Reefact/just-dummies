...
public static partial class Dummy {
    ...
    public static DummyContext WithSeed(int seed) {
        return new DummyContext(seed);
    }
    ...
    ///     test-framework adapter uses: the default snippet is <c>Dummy.Reproducibly(seed, ...)</c>, which points at a
    ...
    public static IDisposable UseSeed(int seed, string replaySnippet) {
        ...
        if (replaySnippet.Trim().Length == 0) { throw new ArgumentException("The replay snippet must be the code a reader copies to replay the run; pass a non-blank snippet, or use the overload without one to name Dummy.Reproducibly(seed, ...).", nameof(replaySnippet)); }
    ...
    ///     <see cref="Dummy" /> stays reproducible: the values still vary between runs (which surfaces accidental
    ...
    ///         with a message naming the <c>Dummy.Reproducibly(seed, ...)</c> call that reproduces the run. Pass your
    ...
    private static void Report(Action<string>? report, int seed) {
        string message = $"[JustDummies] These arbitrary values were seeded with {seed}. Reproduce this run with Dummy.Reproducibly({seed}, ...).";
