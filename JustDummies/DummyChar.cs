...
/// </summary>
/// <remarks>
///     The families mirror <see cref="DummyString" />'s exactly: <see cref="Printable" />, <see cref="NonPrintable" />,
...
///     that empties the pool fails eagerly with a <see cref="ConflictingDummyConstraintException" />. Nothing named
///     reaches past ASCII — a specific alphabet beyond it is <see cref="OneOf" />, whose values are yours.
/// </remarks>
public sealed class DummyChar : IDummy<char>, IHasRandomSource, ICardinalityHint<char>, IPoolInspection<char> {

    #region Statics members declarations

    internal static DummyChar Create(RandomSource source) {
        if (source is null) { throw new ArgumentNullException(nameof(source)); }

        return new DummyChar(source, null, null, null, null, null, null, [], [], []);
    }
    ...
    private DummyChar(RandomSource source,
    ...
    /// <returns>A new generator carrying the added constraint.</returns>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the subtraction leaves no character drawable.</exception>
    public DummyChar WithoutNumeric() {
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyChar OneOf(params char[] values) {
        ...
        return Validated(new DummyChar(_source, _charset, _charsetConstraint, _casing, _casingConstraint, values.Distinct().ToArray(), constraint, _excluded, _exclusions, _subtractions), constraint);
    }
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty.</exception>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyChar Except(params char[] values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }
        if (values.Length == 0) { throw new ArgumentException("At least one value is required.", nameof(values)); }
    ...
    /// <returns>A new generator carrying the added constraint.</returns>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyChar DifferentFrom(char value) {
    ...
    private DummyChar WithCharset(CharacterSet charset, ConstraintCall applying) {
        // Re-declaring the SAME constraint is not a contradiction, so it is a no-op rather than a
        // conflict: the second declaration asks for exactly what the first already guarantees.
        ...
        return Validated(new DummyChar(_source, charset, applying, _casing, _casingConstraint, _allowed, _allowedConstraint, _excluded, _exclusions, _subtractions), applying);
    }

    private DummyChar WithCasing(LetterCasing casing, ConstraintCall applying) {
        // Re-declaring the SAME constraint is not a contradiction, so it is a no-op rather than a
        // conflict: the second declaration asks for exactly what the first already guarantees.
        ...
        return Validated(new DummyChar(_source, _charset, _charsetConstraint, casing, applying, _allowed, _allowedConstraint, _excluded, _exclusions, _subtractions), applying);
    }

    private DummyChar WithExcluded(char[] values, ConstraintCall applying) {
        List<char> excluded = [.. _excluded, .. values];

        return Validated(new DummyChar(_source, _charset, _charsetConstraint, _casing, _casingConstraint, _allowed, _allowedConstraint, excluded, [.. _exclusions, (applying, values.ToArray())], _subtractions), applying);
    }
    ...
    private DummyChar Validated(DummyChar candidate, ConstraintCall applying) {
        ...
        throw ConflictingDummyConstraintException.NoValueRemains(applying, pool);
    }
    ...
    private DummyChar Without(CharacterSet removed, ConstraintCall applying) {
        ...
        return Validated(new DummyChar(_source, _charset, _charsetConstraint, _casing, _casingConstraint, _allowed, _allowedConstraint,
