...
/// <summary>
///     The shared fluent surface of the collection generators. Every collection — a <see cref="DummyList{T}" />,
///     an <see cref="DummyArray{T}" />, an <see cref="DummySequence{T}" /> or an <see cref="DummySet{T}" /> — carries a
...
///     <see cref="ConflictingDummyConstraintException" /> naming both sides. Unconstrained, a collection holds 0 to 8
...
public abstract class DummyCollection<TItem, TResult, TSelf> : IDummy<TResult>, IHasRandomSource
    where TSelf : DummyCollection<TItem, TResult, TSelf> {
    ...
    private protected DummyCollection(RandomSource? source, CollectionState<TItem> state) {
    ...
    public TSelf Containing(TItem value) {
        return With(State.WithContaining(value, ConstraintCall.Of(nameof(Containing), DummyDerivation.Display(value))));
    }
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public TSelf ContainingAny(IDummy<TItem> generator) {
