...
///     generator's domain gates feasibility exactly as it does for a <see cref="DummySet{T}" />: too small a key domain
///     for the requested count fails eagerly with a <see cref="ConflictingDummyConstraintException" />, a genuine
///     shortfall surfaces at generation as an <see cref="DummyGenerationException" />.
/// </summary>
/// <typeparam name="TKey">The key type.</typeparam>
/// <typeparam name="TValue">The value type.</typeparam>
public sealed class DummyDictionary<TKey, TValue> : IDummy<Dictionary<TKey, TValue>>, IHasRandomSource
    where TKey : notnull {
    ...
    private readonly IDummy<TValue>                      _values;

    #endregion

    internal DummyDictionary(RandomSource? source, CollectionState<TKey> keys, IDummy<TValue> values)
        : this(source, keys, values, NoPinnedValues) {
        if (keys is null) { throw new ArgumentNullException(nameof(keys)); }
        if (values is null) { throw new ArgumentNullException(nameof(values)); }
    }

    private DummyDictionary(RandomSource? source, CollectionState<TKey> keys, IDummy<TValue> values, IReadOnlyDictionary<TKey, TValue> pinnedValues) {
        _source       = source;
        _keys         = keys;
        _values       = values;
    ...
    public DummyDictionary<TKey, TValue> NonEmpty() {
    ...
    /// <returns>A new generator carrying the added constraint.</returns>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDictionary<TKey, TValue> Empty() {
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDictionary<TKey, TValue> WithCount(int count) {
    ...
    /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="count" /> is negative.</exception>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDictionary<TKey, TValue> WithMinCount(int count) {
    ...
    /// <exception cref="ArgumentOutOfRangeException">Thrown when <paramref name="count" /> is negative.</exception>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDictionary<TKey, TValue> WithMaxCount(int count) {
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDictionary<TKey, TValue> WithCountBetween(int minimum, int maximum) {
    ...
    ///     generator's domain extends the effective cardinality exactly as <see cref="DummySet{T}" />'s containment
    ...
    /// <returns>A new generator carrying the added constraint.</returns>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDictionary<TKey, TValue> ContainingKey(TKey key) {
        return With(_keys.WithContaining(key, ConstraintCall.Of(nameof(ContainingKey), DummyDerivation.Display(key))));
    }
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDictionary<TKey, TValue> ContainingAnyKey(IDummy<TKey> generator) {
    ...
    /// <returns>A new generator carrying the added constraint.</returns>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDictionary<TKey, TValue> ContainingEntry(TKey key, TValue value) {
        CollectionState<TKey> keys = _keys.WithContaining(key, ConstraintCall.Of(nameof(ContainingEntry), DummyDerivation.Display(key), DummyDerivation.Display(value)));
        ...
        return new DummyDictionary<TKey, TValue>(_source, keys, _values, pinned);
    }
    ...
    private DummyDictionary<TKey, TValue> With(CollectionState<TKey> keys) {
        return new DummyDictionary<TKey, TValue>(_source, keys, _values, _pinnedValues);
    }
