...
/// <summary>
///     A fluent generator of arbitrary <see cref="double" /> values — the same contract as <see cref="DummyInt32" />:
...
///         <c>Dummy.OneOf(double.NaN, 1.0, 2.0)</c>. The generic entry points carry no finiteness rule, by construction.
...
///     </para>
/// </remarks>
public sealed class DummyDouble : IDummy<double>, IHasRandomSource, ICardinalityHint<double>, IPoolInspection<double> {

    #region Statics members declarations

    internal static DummyDouble Create(RandomSource source) {
        if (source is null) { throw new ArgumentNullException(nameof(source)); }

        return new DummyDouble(source, ContinuousIntervalSpec.Unconstrained("Double", V, value => value, ContinuousIntervalSpec.NextUp, -double.MaxValue, double.MaxValue));
    }
    ...
    private DummyDouble(RandomSource source, ContinuousIntervalSpec spec) {
        _source = source;
        _spec   = spec;
    }
    ...
    public DummyDouble Positive() {
        return new DummyDouble(_source, _spec.WithMinimumAbove(0d, ConstraintCall.Of(nameof(Positive))));
    }

    /// <summary>Requires a value strictly less than zero.</summary>
    /// <returns>A new generator carrying the added constraint.</returns>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble Negative() {
        return new DummyDouble(_source, _spec.WithMaximumBelow(0d, ConstraintCall.Of(nameof(Negative))));
    }

    /// <summary>Pins the value to exactly zero.</summary>
    /// <returns>A new generator carrying the added constraint.</returns>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble Zero() {
        return new DummyDouble(_source, _spec.WithMinimum(0d, ConstraintCall.Of(nameof(Zero))).WithMaximum(0d, ConstraintCall.Of(nameof(Zero))));
    }

    /// <summary>Requires a value different from zero.</summary>
    /// <returns>A new generator carrying the added constraint.</returns>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble NonZero() {
        return new DummyDouble(_source, _spec.WithExcluded([0d], ConstraintCall.Of(nameof(NonZero))));
    }
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble GreaterThan(double value) {
        ContinuousIntervalSpec.EnsureFinite(value, nameof(value));
        return new DummyDouble(_source, _spec.WithMinimumAbove(value, ConstraintCall.Of(nameof(GreaterThan), V(value))));
    }
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="value" /> is not finite.</exception>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble GreaterThanOrEqualTo(double value) {
        ContinuousIntervalSpec.EnsureFinite(value, nameof(value));
        return new DummyDouble(_source, _spec.WithMinimum(value, ConstraintCall.Of(nameof(GreaterThanOrEqualTo), V(value))));
    }
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="value" /> is not finite.</exception>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble LessThan(double value) {
        ContinuousIntervalSpec.EnsureFinite(value, nameof(value));
        return new DummyDouble(_source, _spec.WithMaximumBelow(value, ConstraintCall.Of(nameof(LessThan), V(value))));
    }
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="value" /> is not finite.</exception>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble LessThanOrEqualTo(double value) {
        ContinuousIntervalSpec.EnsureFinite(value, nameof(value));
        return new DummyDouble(_source, _spec.WithMaximum(value, ConstraintCall.Of(nameof(LessThanOrEqualTo), V(value))));
    }
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble Between(double minimum, double maximum) {
        ...
        return new DummyDouble(_source, _spec.WithMinimum(minimum, constraint).WithMaximum(maximum, constraint));
    }
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble OneOf(params double[] values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }
        if (values.Length == 0) { throw new ArgumentException("At least one value is required.", nameof(values)); }
        foreach (double value in values) { ContinuousIntervalSpec.EnsureFinite(value, nameof(values)); }

        return new DummyDouble(_source, _spec.WithAllowed(values, ConstraintCall.Of(nameof(OneOf), Join(values))));
    }
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="values" /> is empty or contains a non-finite value.</exception>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble Except(params double[] values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }
        if (values.Length == 0) { throw new ArgumentException("At least one value is required.", nameof(values)); }
        foreach (double value in values) { ContinuousIntervalSpec.EnsureFinite(value, nameof(values)); }

        return new DummyDouble(_source, _spec.WithExcluded(values.ToArray(), ConstraintCall.Of(nameof(Except), Join(values))));
    }
    ...
    /// <exception cref="ArgumentException">Thrown when <paramref name="value" /> is not finite.</exception>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when the constraint contradicts a constraint already declared.</exception>
    public DummyDouble DifferentFrom(double value) {
        ContinuousIntervalSpec.EnsureFinite(value, nameof(value));
        return new DummyDouble(_source, _spec.WithExcluded([value], ConstraintCall.Of(nameof(DifferentFrom), V(value))));
    }
