...
///     commonly when a factory passed to <see cref="DummyExtensions.As{TSource,TResult}" /> or a composer passed to
///     <see cref="Dummy.Combine{T1,T2,TResult}" /> rejects a generated value. Whenever the failing generator draws from
...
///     <see cref="ConflictingDummyConstraintException" /> at declaration time. Reaching this exception therefore
...
/// </remarks>
public sealed class DummyGenerationException : DummyException {
    ...
    /// </summary>
    internal static DummyGenerationException LocalSearchExhausted(string typeName, Replay replay, int budget) {
    ...
    ///     allowed range.
    /// </summary>
    internal static DummyGenerationException GridNudgeExhausted(string typeName, Replay replay) {
    ...
    ///     allowed range.
    /// </summary>
    internal static DummyGenerationException ExclusionNudgeExhausted(string typeName, Replay replay) {
    ...
    ///     offers nothing.
    /// </summary>
    internal static DummyGenerationException EnumDeclaresNoMembers(string enumName) {
        return new DummyGenerationException($"Cannot generate an arbitrary {enumName} value because the enum declares no members.");
    }
    ...
    /// </summary>
    internal static DummyGenerationException EmptyRelativeReference(Replay replay) {
        return new DummyGenerationException("A relative URI with exactly 0 path segments and no query, fragment or root is empty, which is not a valid URI reference. " +
    ...
    /// </summary>
    internal static DummyGenerationException PatternExceedsGenerationLimit(int limit) {
        return new DummyGenerationException($"The pattern produced a string longer than the {limit}-character generation limit. This ceiling guards against runaway expansion; a pattern can reach it " +
    ...
    /// </summary>
    internal static DummyGenerationException PatternVerificationFailed(string attempts) {
        return new DummyGenerationException($"Generation failed: after {attempts} attempts, every value the pattern generator built was rejected by the .NET engine for the same pattern. " +
    ...
    /// </remarks>
    internal static DummyGenerationException FactoryFailed(Func<string> failure, Exception cause, RandomSource? source, bool reproducible) {
        ...
        return new DummyGenerationException(message, replay?.Seed, cause);
    }
    ...
    /// </remarks>
    private static DummyGenerationException NearTheCandidate(string typeName, Replay replay, string diagnostic) {
        return new DummyGenerationException($"Generation failed: no {typeName} value near the drawn candidate satisfies the exclusions. {replay.Guidance}",
                                          replay.Seed,
    ...
    /// <summary>
    ///     Initializes a new instance of the <see cref="DummyGenerationException" /> class.
    /// </summary>
    /// <param name="message">A description of the failed generation.</param>
    public DummyGenerationException(string message) : base(message) { }

    /// <summary>
    ///     Initializes a new instance of the <see cref="DummyGenerationException" /> class wrapping an underlying failure.
    /// </summary>
    /// <param name="message">A description of the failed generation.</param>
    /// <param name="innerException">The underlying failure.</param>
    public DummyGenerationException(string message, Exception innerException) : base(message, innerException) { }

    internal DummyGenerationException(string message, int? seed, Exception innerException) : base(message, innerException) {
        Seed = seed;
    }

    internal DummyGenerationException(string message, int? seed) : base(message) {
        Seed = seed;
    }
