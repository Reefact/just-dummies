...
///         <see cref="ConflictingDummyConstraintException" /> at declaration, like any other emptied domain.
///     </para>
///     <para>
...
///         Order    order    = Dummy.ElementOf(existingOrders).DifferentFrom(theOneAlreadyUsed).Generate();
///         </code>
///     </example>
/// </remarks>
/// <typeparam name="T">The type of the pooled values.</typeparam>
public sealed class DummyOneOf<T> : IDummy<T>, IHasRandomSource, ICardinalityHint<T>, IPoolInspection<T> {
    ...
    internal static DummyOneOf<T> FromPool(RandomSource source, IReadOnlyList<T> values, ConstraintCall declaring) {
        ...
        return new DummyOneOf<T>(source, distinct, distinct, declaring, []);
    }
    ...
    private DummyOneOf(RandomSource source, IReadOnlyList<T> values, IReadOnlyList<T> declared, ConstraintCall declaringConstraint, IReadOnlyList<(ConstraintCall Constraint, T[] Values)> exclusions) {
        _source              = source;
        _values              = values;
        _declared            = declared;
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when no pooled value is left once the excluded ones are removed.</exception>
    public DummyOneOf<T> Except(params T[] values) {
        if (values is null) { throw new ArgumentNullException(nameof(values)); }
    ...
    ///     (<c>Dummy.ElementOf(orders).DifferentFrom(theOneAlreadyUsed)</c>). Semantically equivalent to
    ...
    /// <exception cref="ConflictingDummyConstraintException">Thrown when no pooled value is left once <paramref name="value" /> is removed.</exception>
    public DummyOneOf<T> DifferentFrom(T value) {
    ...
    private DummyOneOf<T> Excluding(IReadOnlyList<T> excluded, ConstraintCall applying) {
        ...
        if (survivors.Length == 0) {
            ...
            throw ConflictingDummyConstraintException.NoValueRemains(applying, emptied);
        }

        return new DummyOneOf<T>(_source, survivors, _declared, _declaringConstraint, [.. _exclusions, (applying, excluded.ToArray())]);
    }
