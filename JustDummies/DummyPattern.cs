...
///     specification: unlike <see cref="DummyString" /> this generator exposes no further shape or length constraints —
...
///         <see cref="DummyGenerationException" /> rather than eagerly at declaration.
///     </para>
///     <para>
///         Only the <b>regular</b> subset of the pattern language is supported (see <see cref="Dummy.StringMatching(string)" />);
...
///         string other     = Dummy.StringMatching(@"^ORD-\d{8}$").DifferentFrom(existing).Generate();
///         IDummy&lt;OrderReference&gt; any = Dummy.StringMatching(@"^ORD-\d{8}$").As(OrderReference.Create);
///         </code>
///     </example>
/// </remarks>
public sealed class DummyPattern : IDummy<string>, IHasRandomSource {

    #region Statics members declarations

    // A nested unbounded quantifier can, in principle, expand super-linearly; this ceiling turns that into a clear
    // DummyGenerationException instead of an out-of-memory. It is far above any realistic format-validation pattern.
    ...
    internal static DummyPattern FromPattern(RandomSource source, string pattern, bool ignoreCase) {
        ...
        return new DummyPattern(source, root, pattern, options);
    }
    ...
    internal DummyPattern(RandomSource source, RegexNode root, string pattern, RegexOptions options) {
        if (source is null) { throw new ArgumentNullException(nameof(source)); }
    ...
    private DummyPattern(DummyPattern origin, IReadOnlyList<string> excluded) {
    ...
    ///     as a seed-bearing <see cref="DummyGenerationException" />, never as a declaration-time conflict: the library
    ...
    public DummyPattern Except(params string[] values) {
    ...
    ///     (<c>Dummy.StringMatching(@"^ORD-\d{8}$").DifferentFrom(existing)</c>). Semantically equivalent to
    ...
    public DummyPattern DifferentFrom(string value) {
    ...
    private string BuildVerified() {
        for (int attempt = 0; attempt < MatchAttemptLimit; attempt++) {
            ...
            try {
                // _root.Append above already refuses, via DummyGenerationException, a pattern whose generation can
        ...
        throw DummyGenerationException.PatternVerificationFailed(V(MatchAttemptLimit));
    }

    private DummyGenerationException Exhausted() {
