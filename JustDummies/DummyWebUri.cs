...
/// </summary>
public sealed class DummyWebUri : IDummy<Uri>, IHasRandomSource {
    ...
    internal DummyWebUri(RandomSource source, UriSpec spec) {
    ...
    public DummyWebUri UsingHttp() {
        return new DummyWebUri(_source, _spec.WithScheme("http", ConstraintCall.Of(nameof(UsingHttp))));
    }

    /// <summary>Pins the scheme to <c>https</c>. Declared once per generator.</summary>
    public DummyWebUri UsingHttps() {
        return new DummyWebUri(_source, _spec.WithScheme("https", ConstraintCall.Of(nameof(UsingHttps))));
    }
    ...
    public DummyWebUri WithHost(string host) {
        return new DummyWebUri(_source, _spec.WithHost(UriSpec.RequireHost(host, nameof(host)), UriSpec.Label(nameof(WithHost), host)));
    }

    /// <summary>Includes arbitrary <c>user:password</c> user-info.</summary>
    public DummyWebUri WithUserInfo() {
        return new DummyWebUri(_source, _spec.WithUserInfo(null, null, UriSpec.Label(nameof(WithUserInfo))));
    }
    ...
    public DummyWebUri WithUserInfo(string user) {
        return new DummyWebUri(_source, _spec.WithUserInfo(UriSpec.RequireUserInfoPart(user, nameof(user)), null, UriSpec.Label(nameof(WithUserInfo), user)));
    }
    ...
    public DummyWebUri WithUserInfo(string user, string password) {
        return new DummyWebUri(_source, _spec.WithUserInfo(UriSpec.RequireUserInfoPart(user, nameof(user)), UriSpec.RequireUserInfoPart(password, nameof(password)), UriSpec.Label(nameof(WithUserInfo), user, password)));
    }

    /// <summary>Includes an arbitrary non-default port.</summary>
    public DummyWebUri WithPort() {
        return new DummyWebUri(_source, _spec.WithPort(null, UriSpec.Label(nameof(WithPort))));
    }
    ...
    public DummyWebUri WithPort(int port) {
        return new DummyWebUri(_source, _spec.WithPort(UriSpec.RequirePort(port, nameof(port)), UriSpec.Label(nameof(WithPort), port)));
    }
    ...
    public DummyWebUri WithPathSegments(int count) {
        return new DummyWebUri(_source, _spec.WithPath(UriPathMode.Exact, UriSpec.RequireSegmentCount(count, nameof(count)), UriSpec.Label(nameof(WithPathSegments), count)));
    }

    /// <summary>Renders the root path (<c>/</c>) with no segments. Declared once per generator.</summary>
    /// <exception cref="ConflictingDummyConstraintException">Thrown when a path constraint is already declared.</exception>
    public DummyWebUri WithoutPath() {
        return new DummyWebUri(_source, _spec.WithPath(UriPathMode.Root, 0, ConstraintCall.Of(nameof(WithoutPath))));
    }

    /// <summary>Includes an arbitrary query string.</summary>
    public DummyWebUri WithQuery() {
        return new DummyWebUri(_source, _spec.WithQuery());
    }

    /// <summary>Includes an arbitrary fragment.</summary>
    public DummyWebUri WithFragment() {
        return new DummyWebUri(_source, _spec.WithFragment());
    }
