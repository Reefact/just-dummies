...
/// </summary>
/// <remarks>
///     Distinct from a <see cref="DerivedDummy{T}" /> built over <c>As(value =&gt; (T?)value)</c>, which is how this
...
internal sealed class NullableDummy<T> : IDummy<T?>, IHasRandomSource, IReproducibilityHint, IComparerSensitiveCardinality<T?>
    where T : struct {

    #region Fields declarations

    private readonly bool          _drawsOnlyFromSource;
    private readonly IDummy<T>       _underlying;
    private readonly RandomSource? _source;

    #endregion

    internal NullableDummy(IDummy<T> underlying) {
        if (underlying is null) { throw new ArgumentNullException(nameof(underlying)); }

        _underlying          = underlying;
        _source              = DummyDerivation.SourceOf(underlying);
        _drawsOnlyFromSource = DummyDerivation.IsReproducible(underlying);
    }
    ...
    long? ICardinalityHint<T?>.DistinctCardinality => DummyDerivation.CardinalityOf(_underlying);
    ...
    long? IComparerSensitiveCardinality<T?>.CardinalityUnderACustomComparer {
        get {
            return _underlying is IComparerSensitiveCardinality<T> sensitive
                       ...
                       : DummyDerivation.CardinalityOf(_underlying);
        }
    }
