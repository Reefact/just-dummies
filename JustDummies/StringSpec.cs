...
internal sealed class StringSpec {
    ...
    ///     guarantee unbuildable, and that is <see cref="ValidateNotBlankIsBuildable" /> — asked at the draw, since
    ///     an anchor declared after this call answers the question in the caller's favour.
    /// </remarks>
    ...
    internal string Generate(RandomSource source) {
        ...
        // The one satisfiability question a later constraint could have answered, so it is asked here — where the
        // specification is whole — rather than on whatever prefix of it happened to be declared first.
        ValidateNotBlankIsBuildable();
    ...
    /// <summary>
    ///     Refuses a shape whose non-blank character has nowhere to come from — <c>Whitespaces().NotBlank()</c>, or
    ///     a length the anchors already fill when none of them carries one.
    /// </summary>
    /// <remarks>
    ///     <para>
    ///         An anchored literal answers for itself: <c>StartingWith("A")</c> already puts a non-blank character
    ///         in every value, so nothing more is demanded of the filler and no alphabet is judged. The literal is
    ///         read, never rejected — a constraint governs what the generator draws, not the text it was handed
    ///         (ADR-0079). Only where the anchors carry nothing does the filler have to supply the character, and
    ///         only then do its alphabet and its room become a satisfiability question.
    ///     </para>
    ///     <para>
    ///         Which is why this runs at the draw and not as each constraint is declared. Every other check here
    ///         answers a question a later call can only narrow, so asking it early is sound. This one does not: an
    ///         anchor declared afterwards <b>satisfies</b> the guarantee, so a chain judged mid-way would refuse
    ///         <c>Whitespaces().NotBlank().StartingWith("A")</c> while accepting the same three constraints written
    ///         in another order. A specification is answered for once it is finished, never on a prefix of itself.
    ///     </para>
    /// </remarks>
    private void ValidateNotBlankIsBuildable() {
        ...
        // Room before alphabet: a shape with no filler position never draws from the pool, so an empty pool cannot
