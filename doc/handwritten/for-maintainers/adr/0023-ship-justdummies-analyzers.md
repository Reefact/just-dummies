# ADR-0023 | Ship first-party JustDummies analyzers, and guard the reproducible async surface with them

🌍 🇬🇧 English (this file) · 🇫🇷 [Français](0023-ship-justdummies-analyzers.fr.md)
...
* The choice of enforcement follows what each mechanism can carry (the same grain as ADR-0014). The type system
...
* ADR-0014 — enforce structural Any conflicts at compile time, value-dependent ones at run time; the "types where
  they can carry the rule, checks where they cannot" grain this decision follows.
* ADR-0010 — name Any's factories after their CLR type; precedent for "make the rule un-break-able rather than merely
  checked", and for TAP-style naming discipline on the surface.
* ADR-0021 — serialize draws on a random source; the sibling reproducibility fix (#310 / #311).
