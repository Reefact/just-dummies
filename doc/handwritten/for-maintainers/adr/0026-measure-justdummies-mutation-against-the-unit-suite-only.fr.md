# ADR-0026 | Mesurer la mutation de JustDummies contre la seule suite unitaire déterministe

🌍 🇫🇷 Français (ce fichier) · 🇬🇧 [English](0026-measure-justdummies-mutation-against-the-unit-suite-only.md)

**Statut :** Accepté
**Proposé :** 2026-07-27
**Accepté :** 2026-07-31
**Décideurs :** Reefact
**Enregistré à l'origine dans `Reefact/first-class-errors` sous le numéro ADR-0047.**

## Contexte

L'ADR-0022 a configuré le run de mutation de JustDummies pour utiliser **les deux** suites de tests
...
* **Un oracle non-déterministe.** Un verdict de mutation répond à « un test de l'oracle échoue-t-il sur ce
  ...
  **tué sur les tirages d'un run et survivant sur ceux d'un autre**. Le score de mutation reflète alors la
  ...
  `"coverage-analysis": "off"` existe pour protéger (ADR-0022).
...
* Un comportement épinglé **uniquement** par un property test, sans aucun test unitaire l'affirmant,
  ...
  d'ajouter un test unitaire — l'ADR-0019 régit déjà quelle suite possède quel cas.
...
* ADR-0022 — Contrôler les pull requests sur le score de mutation du diff : le run dont ceci restreint
  l'oracle.
* ADR-0019 — Séparer le banc de tests de JustDummies entre une suite d'exemples et une suite de
...
* ADR-0025 — Rendre la porte de mutation par pull request consultative : la décision sœur de
  vitesse/blocage.
