# ADR-0028 | Drop the JustDummies generator from the per-pull-request mutation matrix

🌍 🇬🇧 English (this file) · 🇫🇷 [Français](0028-drop-the-justdummies-generator-from-the-per-pull-request-mutation-matrix.fr.md)

**Status:** Accepted
**Proposed:** 2026-07-28
**Accepted:** 2026-07-28
**Decision Makers:** Reefact
**Originally recorded in `Reefact/first-class-errors` as ADR-0049.**

## Context

ADR-0022 gates every pull request on the mutation score of what it changed. `justdummies-mutation.yml`
...
* **`"coverage-analysis": "off"` is mandatory, not tuning.** Under the MTP runner Stryker's test
  ...
  an accuracy decision (ADR-0022, and `mutation.en.md`, "Two settings that are not tuning knobs"), not a
...
* **ADR-0025 already removed its authority.** The per-PR gate is advisory; the enforced bar is the weekly
...
Considered because it needs no ADR — it is an implementation detail of ADR-0022's "gate the diff", and
...
which is among the defects mutation testing catches best (ADR-0024 is the decision those guards
implement).
...
* ADR-0022 — Gate pull requests on the mutation score of the diff: the decision this narrows.
* ADR-0025 — Make the per-pull-request mutation gate advisory: why the leg has no authority to lose.
* ADR-0026 — Measure JustDummies mutation against the unit suite only: the previous attempt to make this
  leg affordable.
* ADR-0024 — Guard public and internal arguments against null: the guards `ignore-mutations` would stop
  testing.
