# ADR-0031 | Tirer les nombres arbitraires dans une magnitude ordinaire
...
**Accepté :** 2026-07-28
**Décideurs :** Reefact
**Enregistré à l'origine dans `Reefact/first-class-errors` sous le numéro ADR-0052.**
...
L'ADR-0029 a consigné la règle homologue pour les *tailles* : un dummy est petit à moins que quelque chose
...
tirage non contraint est encore libre d'être fixé — l'acquis sur lequel s'appuyait l'ADR-0020.
...
* **Rogner plutôt que remplacer est ce qui garde la règle honnête.** Un appelant qui nomme une magnitude —
  ...
  elle refuse seulement de la viser. C'est la règle de l'ADR-0029 pour les tailles, transposée telle
...
* ADR-0029 — Let a size maximum cap without steering the draw : le même principe appliqué aux tailles,
...
* ADR-0019 — Split the JustDummies test bed between example and property suites : pourquoi la règle de la
