# ADR-0033 | Décider la surface de contraintes d'un générateur par constructif contre rejectif, et non par terminalité

🌍 🇫🇷 Français (ce fichier) · 🇬🇧 [English](0033-decide-a-constraint-surface-by-constructive-versus-rejective.md)

**Statut :** Accepté
**Proposé :** 2026-07-28
**Accepté :** 2026-07-28
**Décideurs :** Reefact
**Enregistré à l'origine dans `Reefact/first-class-errors` sous le numéro ADR-0054.**

Supersède l'[ADR-0009](0009-draw-arbitrary-strings-from-an-explicit-terminal-set.fr.md).
...
* Les deux coûts que l'ADR-0009 évitait ne sont pas symétriques. Longueur, préfixe, suffixe, valeurs contenues, famille
...
* **« Terminal » décrivait le type retourné, pas le domaine : impossible d'en raisonner.** Les ADR-0009 et ADR-0008 ont
...
* **Un ensemble de valeurs fourni par l'appelant est un domaine, pas une mise en forme : le coût combinatoire que
  l'ADR-0009 refusait ne se présente jamais.** L'argument de l'ADR-0009 était qu'un ensemble explicite devrait être
...
* **Une contrainte rejective ne demande aucune machinerie nouvelle et ne crée aucune exception nouvelle au « construit,
  ...
  exception est déjà documentée. Sur un motif, la boucle qui porterait l'exclusion est celle que l'ADR-0027 fait déjà
...
le sous-ensemble régulier est délibérément maison et petit (ADR-0008), pour une combinaison qu'aucun cas d'usage
...
* Passer l'ADR-0009 au statut *Superseded* avec un lien vers celle-ci, une fois ce document accepté.
* Décider si l'ADR-0008 a besoin d'un successeur : sa décision — générer depuis un sous-ensemble régulier maison —
...
* ADR-0009 — Tirer des chaînes arbitraires depuis un ensemble de valeurs explicite et terminal : la décision que
  celle-ci supersède, et le *Risque* qu'elle consignait sur les appelants attendant la composabilité.
* ADR-0008 — Générer les chaînes qui matchent depuis un sous-ensemble régulier maison : le précédent de générateur
  terminal sur lequel l'ADR-0009 s'est alignée, et la raison pour laquelle une contrainte constructive sur un motif
  reste refusée.
* ADR-0027 — Garantir qu'une valeur regex générée matche son motif, par retirage borné : la boucle que rejoint une
  exclusion.
