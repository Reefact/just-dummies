...
The offset arithmetic, the instant-tightening bounds, and the draw are implementation, documented in the `AnyDateTimeOffset` code and the Dummies user documentation — not here.
...
* Document `WithOffset`/`WithOffsetBetween` in the Dummies readme and the builder documentation (done in the implementing pull request).
...
* `AnyDateTimeOffset` in the `Dummies` project; the Dummies NuGet readme.
