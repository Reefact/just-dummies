# ADR-0038 | Guard the recipe-versus-value boundary with analyzers where the type system cannot reach it

🌍 🇬🇧 English (this file) · 🇫🇷 [Français](0038-guard-the-recipe-versus-value-boundary-with-analyzers.fr.md)

**Status:** Accepted
**Proposed:** 2026-07-29
**Accepted:** 2026-07-31
**Decision Makers:** Reefact
**Originally recorded in `Reefact/first-class-errors` as ADR-0059.**

## Context

* ADR-0006 removed the 28 implicit conversions from JustDummies generators to their generated types, making
...
* ADR-0023 established first-party JustDummies analyzers as the repository's answer to a mistake the type system
  cannot express, and its own follow-up invites applying that pattern to future such mistakes. ADR-0014 draws the
...
* The decision ADR-0006 took is untouched and remains right: `Generate()` stays the sole materialization, and no
  implicit conversion returns. What this ADR revises is one **prediction** ADR-0006 made about the world after that
...
* The analyzer ADR-0006 declined and the analyzers decided here are not the same instrument. The rejected one was
  ...
  removal left open. ADR-0006's argument against the first does not reach the second.
* The enforcement point follows what each mechanism can know, the same grain as ADR-0014 and ADR-0023. C# cannot
...
* Severity follows the failure mode rather than the family. A generator rendered as text is a silent green — the
  build succeeds, the test passes, the assertion is meaningless — which is the case ADR-0023 already judged worth
...
### Leave it to documentation, as ADR-0006's follow-up prescribed
...
Rejected because it reintroduces exactly what ADR-0006 removed, and for a worse reason: the conversion is
...
* ADR-0006's factual claim is corrected in the record rather than left to be discovered by whoever hits it, and the
...
* Supersede nothing: ADR-0006's decision stands unchanged, and its status is the maintainer's to revisit if the
...
* ADR-0006 — materialize dummies only through `Generate()`; the decision this one leaves standing and whose
