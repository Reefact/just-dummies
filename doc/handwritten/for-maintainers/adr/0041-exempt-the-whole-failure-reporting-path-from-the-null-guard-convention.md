# ADR-0041 | Exempt the whole failure-reporting path from the null-guard convention

🌍 🇬🇧 English (this file) · 🇫🇷 [Français](0041-exempt-the-whole-failure-reporting-path-from-the-null-guard-convention.fr.md)

**Status:** Accepted
**Proposed:** 2026-07-30
**Accepted:** 2026-07-30
**Decision Makers:** Reefact
**Originally recorded in `Reefact/first-class-errors` as ADR-0064.**

Supersedes [ADR-0024](0024-guard-public-and-internal-arguments-against-null.md).

## Context

ADR-0024 requires every public and internal member to reject a `null` non-nullable reference
...
ADR-0040 made the library throw through factories named after the failure, and one of those
...
ADR-0024 forbids, one call frame earlier: a `null` would now surface as `ArgumentNullException` from
...
ADR-0024's rule stands in full, with its exemption widened from exception types to any type that
...
the rest is exactly what ADR-0040's factories create.
...
What the code did before this ADR, and what the convention forced. It recreates the masking ADR-0024
exists to prevent, one frame away from where ADR-0024 forbids it. Rejected on the merits, not on
...
nothing checks — the signature ADR-0040 rejected. The exemption exists so the call site can stay
readable.
...
the prose-in-business-code ADR-0040 was written to end. Rejected there, rejected here.

## Consequences

### Positive

* The hazard ADR-0024 identified is now covered wherever it occurs, instead of wherever a type
...
* [ADR-0024](0024-guard-public-and-internal-arguments-against-null.md) — the rule this supersedes
  and restates, and the exemption this widens.
* [ADR-0040](0040-throw-the-library-s-own-exceptions-through-named-factories.md) — the named
