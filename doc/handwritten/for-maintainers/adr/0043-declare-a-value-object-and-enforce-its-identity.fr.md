# ADR-0043 | Déclarer un objet-valeur par un attribut, et faire respecter son identité par convention

🌍 🇫🇷 Français (ce fichier) · 🇬🇧 [English](0043-declare-a-value-object-and-enforce-its-identity.md)

**Statut :** Accepté
**Proposé :** 2026-07-30
**Accepté :** 2026-07-31
**Décideurs :** Reefact
**Enregistré à l'origine dans `Reefact/first-class-errors` sous le numéro ADR-0066.**

## Contexte

La bibliothèque porte trois valeurs faites pour être comparées ou transportées par leur contenu plutôt que par
l'instance qu'on tient : une contrainte déclarée (ADR-0042), la paire d'un sujet blâmé et de ce qu'il affirme, et ce
...
garde null d'ADR-0024 découvre les membres au lieu de les nommer, et ADR-0041 déclare son exemption par
`[BuiltOnTheFailurePath]` plutôt que de l'inférer. ADR-0035 consigne ce qu'il advient d'une règle dans ce dépôt quand
...
qu'un attribut consigne — le raisonnement même qu'ADR-0041 a appliqué à sa propre exemption plutôt que de l'inférer
...
* Examiner si les valeurs de `FirstClassErrors` — qui portent déjà leurs identités — doivent se déclarer de la même
  façon ; les deux assemblages ne peuvent pas partager l'attribut, JustDummies étant autonome par ADR-0003.
...
* [ADR-0035](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0056-state-the-coding-rules-where-an-agent-can-act-on-them.fr.md) — une règle sur laquelle rien ne peut
  agir dérive.
* [ADR-0038](0038-guard-the-recipe-versus-value-boundary-with-analyzers.fr.md) — quand l'analyseur est l'instrument.
* [ADR-0041](0041-exempt-the-whole-failure-reporting-path-from-the-null-guard-convention.fr.md) — un marqueur
  consignant une décision plutôt que de l'inférer.
* [ADR-0042](0042-carry-a-declared-constraint-as-a-value-object.fr.md) — la valeur dont l'égalité a forcé la
  question.
