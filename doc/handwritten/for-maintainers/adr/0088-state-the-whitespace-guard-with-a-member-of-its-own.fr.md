...
**Et cette satisfiabilité se tranche sur la spécification achevée, non à mesure que les appels arrivent.** Tous
les autres contrôles de ce générateur répondent à une question qu'un appel ultérieur ne peut que resserrer :
la poser tôt est donc correct. Celui-ci est d'une autre nature — une ancre déclarée ensuite **relâche** la
contrainte au lieu de la durcir, si bien qu'une chaîne jugée en cours de route est jugée sur un préfixe
d'elle-même. Laissé eager, il refusait `Whitespaces().NotBlank().StartingWith("A")` tout en acceptant les mêmes
trois appels dans un autre ordre — alors que l'ADR-0079 consigne l'indépendance à l'ordre comme une propriété de
cette surface, obtenue là-bas en supprimant l'échec plutôt qu'en le différant. Le supprimer n'est pas possible
ici : un alphabet de remplissage sans caractère non blanc, et aucune ancre pour en fournir un, est réellement
insatisfiable. La question se déplace donc au seul moment où la réponse ne peut plus changer. Ce qu'un appelant
perd, c'est le refus au niveau de l'appel fautif plutôt qu'au tirage. Ce qu'il gagne, c'est qu'une chaîne
signifie ce qu'elle énonce quel que soit l'ordre d'écriture — la plus forte des deux promesses, et celle que la
documentation faisait déjà.
...
* Un membre de plus sur la surface chaîne, et chaque mécanisme auquel cette surface répond gagne une entrée avec
