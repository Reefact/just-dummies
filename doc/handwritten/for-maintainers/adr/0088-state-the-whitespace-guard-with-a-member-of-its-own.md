...
**And that satisfiability is settled on the finished specification, not as each call arrives.** Every other check
this generator makes answers a question a later call can only narrow, so asking it eagerly is sound. This one is
different in kind: an anchor declared afterwards **relaxes** the constraint rather than tightening it, so a chain
judged mid-way is judged on a prefix of itself. Left eager, it refused `Whitespaces().NotBlank().StartingWith("A")`
while accepting the same three calls in another order — and ADR-0079 records order-independence as a property this
surface holds, obtained there by removing the failure rather than deferring it. Removing it is not available here,
because a filler alphabet with no non-blank character and no anchor to supply one is genuinely unsatisfiable; so
the question moves to the one moment the answer cannot change again. What a caller loses is the refusal arriving
at the offending call rather than at the draw. What a caller gains is that the chain means what it says whatever
order it was written in, which is the stronger of the two promises and the one the documentation already made.
...
* One more member on the string surface, and every mechanism that surface answers to gains an entry with it —
...
* The decision has two halves and they ship on different trains. The member is on the library's; the scaffolder
  reading each whitespace spelling as it — `Guards.cs` and `LibraryGuards.cs`, which still emit `NonEmpty()` —
  is on the `cli` train and lands with it. Until then the guard is read as the floor it outlives, which is the
  defect this record opens on.
