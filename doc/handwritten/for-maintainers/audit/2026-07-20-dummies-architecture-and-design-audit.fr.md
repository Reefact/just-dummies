...
1. **Critique — `DummyDecimal` ne peut jamais générer la moitié haute de sa plage.** Une fraction censée
   être uniforme dans [0, 1) est construite à partir de trois tirages de 31 bits divisés par un
   dénominateur de 96 bits, et plafonne près de 0,5 ; `Dummy.Decimal().Between(0m, 100m)` ne dépasse
   jamais ~49,9999 (`DecimalIntervalSpec.cs:145`).
2. **Majeur — le « nudge » d'exclusion de `DummySingle`/`DummyHalf` se bloque.** La marche
   ...
   `Dummy.Half().Between((Half)1f, (Half)1.001f).DifferentFrom((Half)1f)` lève une
   `DummyGenerationException` pour ~la moitié des graines (`ContinuousIntervalSpec.cs:189`).
3. **Majeur — une plage de classe regex se terminant à `￿` boucle à l'infini.** La boucle
   ...
   `Dummy.StringMatching(@"[ -￿]")` ne rend jamais la main (`RegexParser.cs:398`).
...
Au-delà des défauts, les constats significatifs sont : la surface `Dummy`/`DummyContext` recopiée à la main
...
lorsque l'[ADR-0006 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0006-supply-arbitrary-test-values-from-a-seedable-source.fr.md) a été remplacée) ; la **cible netstandard2.0 n'est jamais exécutée par la propre
...
surprenantes sont un combinateur de choix *de premier niveau* (`Dummy.OneOf<T>(params T[])` /
`Dummy.ElementOf(...)`) et des contraintes d'exclusion sur `DummyString` — le seul générateur scalaire qui
en soit dépourvu.
...
moment de `Generate()` — est ce qui permet à une recette construite hors d'un `Dummy.Reproducibly(...)`
...
exception dans `.As(...)` produit une `DummyGenerationException` nommant la valeur générée *et* la graine
(`DummyDerivation.cs:59-73`) ; une saturation de collection distincte fait de même
...
représentable voisine), et l'évitement de collision d'`DummyGuid` (une incrémentation avec retenue sur
toute la largeur qui se termine de façon prouvable — `DummyGuid.cs:27-36`). Chaque mode d'échec produit un
message actionnable, porteur de graine, plutôt qu'un blocage.

### 3.5 Le soin aux bords

De petites choses qui révèlent le niveau d'exigence : `DummyDateTime.OneOf` se souvient des valeurs
originales de l'appelant pour que l'aller-retour ordinal ne normalise pas silencieusement le
`DateTimeKind` (`DummyDateTime.cs:124-131`) ; l'agencement d'une collection est mélangé par Fisher-Yates
...
déborder sur des minima déclarés énormes ; la covariance d'`IDummy<out T>` fait que les interfaces de
...
relatives à l'horloge (`DummyDateTime` n'a pas d'`InThePast()`) est documenté à chaque endroit où
...
**(a) `DummyDecimal` n'atteint jamais la moitié haute d'une plage — critique.**
...
`[min, mid)`. Le maximum inclusif documenté sur `DummyDecimal.Between` (`DummyDecimal.cs:112`) est
inatteignable — tout comme tout ce qui est au-dessus du milieu. Reproduit indépendamment pour cet
audit : le maximum de 200 000 tirages de `Dummy.Decimal().Between(0m, 100m)` valait **49,99992…**.

Pourquoi cela compte au-delà de l'évidence : un test utilisant `Dummy.Decimal().Between(0m, 100m)` pour
...
**(b) Le nudge d'exclusion de `DummySingle`/`DummyHalf` se bloque sur des specs satisfiables — majeur.**
...
*conscient du type* que `DummySingle`/`DummyHalf` fournissent précisément pour avancer dans leur propre
échelle de valeurs représentables (`DummySingle.cs:20`, `DummyHalf.cs:22`) — et que les chemins à borne
...
`Dummy.Half().Between((Half)1f, (Half)1.001f).DifferentFrom((Half)1f).Generate()` a levé une
`DummyGenerationException` pour **250 graines sur 500** ; le scénario `DummyDouble` identique ne lève jamais
...
`Dummy.StringMatching(@"[ -￿]")` n'a pas rendu la main en cinq secondes (blocage dur), alors que le même
...
ordinaire : `Dummy.StringMatching(@"(?<a>y)?(?<-a>x)")` génère `"x"`, que le vrai moteur ne reconnaît
...
`RegexAlphabet.cs:3-9`, `DummyPattern.cs:21` et `Dummy.Pattern.cs:23` affirment tous que chaque terminal se
...
* **`Dummy` ↔ `DummyContext`** : chaque point d'entrée scalaire existe deux fois (21 sur la cible
  netstandard2.0, 26 sur net8.0, en comptant les deux surcharges de `StringMatching`) —
  `Dummy.Primitive.cs:11-224`, `Dummy.Pattern.cs:34-52` et `Dummy.Uri.cs:13` face à `DummyContext.cs:51-305`.
  ...
  de leurs opérandes, ce qui est élégant), mais un nouveau type scalaire ajouté à `Dummy` et oublié sur
  `DummyContext` compilerait, passerait les 222 tests, et livrerait un trou dans la surface déterministe.
  La dérive de formulation est déjà visible à l'intérieur d'`DummyContext` lui-même (deux tournures
  différentes du déterminisme selon les fabriques ; sa doc de `Guid()` mentionne `Dummy.Reproducibly`,
...
* **Les quatorze générateurs numériques** sont des clones identiques à l'octet près modulo substitution
  ...
  lui-même — mais trois résumés XML disent « Same constraint algebra as `DummyInt32` » sur des générateurs
...
promise ni écartée ; et le contrat entier a perdu son ancrage ADR lorsque l'[ADR-0006 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0006-supply-arbitrary-test-values-from-a-seedable-source.fr.md) a été remplacée.
...
`Arrange` avec `JustDummies.Dummy` via référence de projet et via les fabriques de Testing, si bien que
...
### [ADR-0006 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0006-supply-arbitrary-test-values-from-a-seedable-source.fr.md) — Fournir des valeurs de test arbitraires depuis une source unique semable *(Remplacée)*
...
timing pré-1.0 énoncé comme tel. Elle a de plus manifestement orienté des travaux ultérieurs (l'[ADR-0026 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0026-rebase-testing-arbitrary-values-on-dummies.fr.md)
...
*porteuse pour l'[ADR-0026 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0026-rebase-testing-arbitrary-values-on-dummies.fr.md) Acceptée* (dont `ErrorCodeFactory` est bâti sur `StringMatching`) — tant que
...
### [ADR-0026 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0026-rebase-testing-arbitrary-values-on-dummies.fr.md) — Rebaser les valeurs arbitraires du paquet de test sur JustDummies *(Acceptée)*
...
précision :** (1) le texte de décision dit que chaque fabrique expose « an `IDummy<T>` generator through a
distinct method where composition is needed » — aucune fabrique n'expose une telle méthode aujourd'hui
(vérifié : zéro occurrence d'`IDummy` dans les sources de `FirstClassErrors.Testing`). YAGNI défendable,
...
1. **Le contrat de déterminisme de JustDummies n'a pas d'ADR acceptée.** La source ambiante `AsyncLocal`,
   ...
   joyau de la couronne — a été décidée dans l'[ADR-0006 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0006-supply-arbitrary-test-values-from-a-seedable-source.fr.md), désormais Remplacée *et* cadrée sur
   FirstClassErrors.Testing ; la décision de l'[ADR-0026 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0026-rebase-testing-arbitrary-values-on-dummies.fr.md) porte sur le rebasage de Testing, pas sur le
   ...
   reproductibilité optionnelle ») reprenant la justification de l'[ADR-0006 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0006-supply-arbitrary-test-values-from-a-seedable-source.fr.md) et réglant, dans le même
...
|---|---|---|
| 0006 (historique) | Remplacée | **Conforme et dépassée.** Le contrat de graine hérité (local au contexte, déterminisme optionnel, rapport de graine) est implémenté fidèlement ; JustDummies ajoute l'`DummyContext` isolé que l'ADR d'origine ne faisait qu'anticiper. |
...
| 0013 | Acceptée | **Conforme, vérifiée en détail** — gate immédiat net des crédits `Containing` hors domaine, comptage conservateur de `ContainingAny`, arithmétique à l'abri du débordement, budget borné, les deux canaux d'échec. **Une déviation mineure :** le message de saturation promet *inconditionnellement* le rejeu `Dummy.Reproducibly({seed}, …)` (`CollectionState.cs:246-254` ; la garde `seed is not null` est du code mort — la graine n'y peut jamais être nulle). Pour un générateur d'éléments **étranger** dont les tirages ignorent la source ambiante, cette promesse est fausse ; l'ADR dit que les échecs sont « explicites et reproductibles ». Qualifier le message quand le générateur d'éléments ne porte aucune source de la bibliothèque. |
| 0015 | Acceptée | **Conforme exactement** — arités 2–8, pas plus ; suppressions localisées avec justifications renvoyant à l'ADR (`Dummy.Combine.cs:214-215`, `266-267`) ; plafond documenté sur la surcharge d'arité 8. |
...
| 0026 | Acceptée | **Conforme sur chaque clause exécutée** — moteur unique, scope de graine unique, `Testing.Dummy` supprimé, fabriques livrées, horloge/ids sur le contexte ambiant, docs mises à jour EN/FR. La moitié « distinct `IDummy<T>` method » non implémentée et la prémisse de risque mal énoncée sont consignées au §5. |
...
vers le bas). Les points de composition — `.As(factory)`, `Dummy.Combine(...)`, les fabriques de
collection — sont tous définis sur l'`IDummy<out T>` à un seul membre, qui est aussi petit qu'une interface
...
La **hiérarchie de collections** est un CRTP propre comme un manuel : `DummyCollection<TItem, TResult,
...
L'exception est `DummyDictionary`, qui ne peut pas hériter de la base (son élément est une paire) et donc
**duplique toute la façade de compte mot pour mot** (~60 lignes, `DummyDictionary.cs:51-113`) et n'offre
...
`IDummy<T>` est public, donc n'importe qui peut implémenter un générateur et le composer via
...
`Dummy.Reproducibly` ses valeurs ne rejouent pas, et (b) ne peut pas annoncer un domaine fini — une
...
qui manque est un paragraphe honnête sur la doc XML d'`IDummy<T>` disant aux implémenteurs où ils se
...
**Pour les mainteneurs**, ajouter un nouveau type scalaire touche 6 à 9 fichiers (générateur, `Dummy`,
`DummyContext`, tests, docs utilisateur EN/FR, README du paquet, `justdummies-check` si net8-only,
...
d'échec (`Dummy.Combine.cs:33` et al.). `Dummy.Combine(Dummy.WithSeed(1).Int32(), Dummy.WithSeed(2).Int32(), throwing)`
échoue avec « seeded with 1; reproduce with `Dummy.Reproducibly(1, …)` » — doublement faux : la graine 2
...
(ambiante → « reproduce with `Dummy.Reproducibly({seed}, …)` » ; fixe → « ce générateur tire de
`Dummy.WithSeed({seed})`, qui rejoue déjà de lui-même »), et faire collecter à `Combine` les sources
...
`NewSeed() = Guid.NewGuid().GetHashCode()` (usage tolérant aux collisions, analysé dans l'[ADR-0006 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0006-supply-arbitrary-test-values-from-a-seedable-source.fr.md)) ;
...
(pas d'astuce d'auto-cast, borne `TSelf` imposée). ISP : `IDummy<T>` à membre unique ; les deux membres
...
inclusive/exclusive identique, tandis qu'`DummyTimeSpan` — une magnitude, pas un instant — garde
correctement l'algèbre numérique complète, y compris `Positive`/`Negative`/`Zero` ; `DummyChar` porte les
familles de caractères plus le trio d'exclusion ; `DummyGuid` a
`NonEmpty`/`Empty`/`OneOf`/`Except`/`DifferentFrom` ; `DummyEnum` a le trio d'exclusion avec validation des
...
* **`DummyString` est le seul générateur scalaire sans contraintes d'exclusion** — pas d'`OneOf`, pas
  ...
  string other = Dummy.String().NonEmpty().Generate();     // pourrait être égal à l'existant !
  // Proposé :
  string other = Dummy.String().NonEmpty().DifferentFrom(existing).Generate();
  ```

* **`DummyDictionary` abandonne `Containing`/`ContainingAny`** et duplique la façade de compte (§7.1).
  `ContainingKey(TKey)` chevaucherait la machinerie d'état de clés existante sans changement.
* **`Dummy.Bool()` est la seule déviation de la convention de fabriques aux noms CLR** (`Int32`, `SByte`,
...
Le point d'entrée statique `Dummy.` rend toute la surface scalaire découvrable en une frappe, et les
...
décision doit tenir. `DummyContext` ne recopie que les scalaires — la composition hérite du contexte via
...
philosophie l'entend. Les fabriques aux noms de type CLR (`Dummy.Int32()`, pas `Dummy.Int()`) sont cohérentes
avec les noms de type des générateurs (`DummyInt32`) et contournent les restrictions de mots-clés C# ;
...
travers les quatre moteurs (~910 lignes), et le miroir scalaire `Dummy`/`DummyContext` (~300 lignes riches en
...
1. *Parité du miroir :* chaque méthode statique publique `Dummy` renvoyant un type de générateur a une
   contrepartie d'instance `DummyContext` de nom/signature/type de retour identiques, par TFM (~20 lignes ;
...
des tests de non-régression qui encodent l'historique des bugs (le test de course d'`DummyGuid` qui court
...
`WithSeed`) ; un test de limite de génération pour `DummyPattern` (actuellement non testée) ; des tests
...
de dossiers (`Dummy*` = générateurs, `*Spec` = moteurs, `Regex*` = sous-système de motifs) ; regrouper en
...
`Dummy` + `DummyContext` + données de parité + tests + docs EN/FR + README du paquet + éventuellement
...
**1. Un combinateur de choix de premier niveau : `Dummy.OneOf<T>(params T[])` et
`Dummy.ElementOf<T>(IReadOnlyList<T>)`.**
...
distinctes), recopié sur `DummyContext`. Qui en bénéficie : chaque consommateur, chaque semaine. Coût : un
...
* **Générateur `Uri`** (`Dummy.Uri().UsingHttps().WithHost("example.com")`) — le seul type BCL de type
...
* **`WithChars(string pool)` / alphabet personnalisé sur `DummyString`** — aujourd'hui le texte non-ASCII
...
* **`ContainingKey(TKey)` sur `DummyDictionary`** (§7.1/§8.2) — referme d'un coup un trou d'API, une
  duplication et un trou de test.
* **Combinaisons d'enum [Flags], en opt-in** (`Dummy.Enum<Permissions>().AllowingCombinations()`) —
...
* **`WithOffset`/contrôle d'offset sur `DummyDateTimeOffset`** — la dimension d'offset est actuellement
...
* **Un adaptateur de graine pour framework de test** (`[ReproducibleFact]`) — anticipé par les suivis de
  l'[ADR-0006 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0006-supply-arbitrary-test-values-from-a-seedable-source.fr.md), abandonné lors du rebasage, remplacé par rien. JustDummies zéro-dépendance ne peut pas
...
`Combine(Dummy.Int32().Between(0,99), …, (ma,mi,pa) => new Version(ma,mi,pa))` ; faible fréquence) ;
...
`Stream`, blobs `byte[]` (toutes des recettes d'une ligne sur la surface existante — `ArrayOf(Dummy.Byte())`
...
3. **Ajouter les gardes de parité** (§9.2) : test de miroir `Dummy`↔`DummyContext`, test de matrice
   d'algèbre.
...
7. **Gardes d'ingénierie de release** : référence d'API publique (`PublicApiAnalyzers`) et
   ...
   clarifications de l'[ADR-0026 (first-class-errors)](https://github.com/Reefact/first-class-errors/blob/main/doc/handwritten/for-maintainers/adr/0026-rebase-testing-arbitrary-values-on-dummies.fr.md) dans la référence d'implémentation ; enrichir ou adoucir les renvois à la
   référence d'implémentation des ADR-0004/0015.
8. **Livrer les deux fonctionnalités Indispensables** (§10) : `Dummy.OneOf<T>`/`Dummy.ElementOf<T>`, et les
   exclusions de chaîne (`DifferentFrom`/`Except` sur `DummyString`).
9. **`DummyDictionary`** : extraire la façade de compte partagée ; ajouter `ContainingKey`.
...
|---|---|---|
| 1 — Corriger les défauts reproduits | [#206](https://github.com/Reefact/first-class-errors/issues/206) DummyDecimal moitié haute · [#207](https://github.com/Reefact/first-class-errors/issues/207) nudge Single/Half · [#208](https://github.com/Reefact/first-class-errors/issues/208) blocage U+FFFF · [#209](https://github.com/Reefact/first-class-errors/issues/209) groupes d'équilibrage · [#210](https://github.com/Reefact/first-class-errors/issues/210) bords regex mineurs | 0 |
...
| 8 — Livrer les Indispensables | [#223](https://github.com/Reefact/first-class-errors/issues/223) Any.OneOf/ElementOf · [#224](https://github.com/Reefact/first-class-errors/issues/224) exclusions DummyString | 1 |
