# `genany-sweep` workflow

🌍 🇬🇧 English (this file) · 🇫🇷 [Français](genany-sweep.fr.md)

> Maintainer documentation — part of the [workflow reference](README.md).
> Not part of the user documentation under `doc/`.

**Workflow file:** [`.github/workflows/genany-sweep.yml`](../../../../.github/workflows/genany-sweep.yml)
**Suite:** [`JustDummies.GenAny.UnitTests/GenerativeSweepTests.cs`](../../../../JustDummies.GenAny.UnitTests/GenerativeSweepTests.cs)
**Bench:** [`JustDummies.GenAny.UnitTests/Sweep/`](../../../../JustDummies.GenAny.UnitTests/Sweep/)
...
[`GuardCorpus.cs`](../../../../JustDummies.GenAny.UnitTests/GuardCorpus.cs) between them produced
...
| 5 | a draw produces a value, **or** is refused with `AnyGenerationException` | a finding |
...
Every one of the 352 draw failures in the August survey was an `AnyGenerationException` — the library
...
[`SweepDefects.cs`](../../../../JustDummies.GenAny.UnitTests/Sweep/SweepDefects.cs), neither fixed in
the change that installed the bench.

**`cardinality-hint-lost-through-as` (55 shapes).** `Any.SetOf(Any.Boolean())` gates the set at two
elements, because `AnyBoolean` carries `ICardinalityHint<bool>` and `AnySet` reads it (ADR-0004).
`Any.SetOf(Any.Boolean().As(value => (bool?)value))` does not: `AnyExtensions.As` returns a
`DerivedAny<TResult>` carrying the random source and the reproducibility of what it wraps, and nothing
...
**`nested-collection-loses-its-declared-interface` (48 shapes).** `Any.SetOf(…)` is typed
`IAny<HashSet<T>>` and `Any.ListOf(…)` is `IAny<List<T>>`, so a collection *of* one of those carries
...
`JustDummies.GenAny.UnitTests`. It cannot find what the product finds. It exists so the apparatus
...
JUSTDUMMIES_SWEEP=1 dotnet test JustDummies.GenAny.UnitTests    # the whole product
```
...
* `JustDummies.GenAny.UnitTests/Sweep/sweep-baseline.tsv` — **committed**, and checked by the full
  sweep.
