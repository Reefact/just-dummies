# Workflow `gendummy-sweep`

🌍 🇫🇷 Français (ce fichier) · 🇬🇧 [English](gendummy-sweep.en.md)
...
**Fichier du workflow :** [`.github/workflows/gendummy-sweep.yml`](../../../../.github/workflows/gendummy-sweep.yml)
**Suite :** [`JustDummies.GenDummy.UnitTests/GenerativeSweepTests.cs`](../../../../JustDummies.GenDummy.UnitTests/GenerativeSweepTests.cs)
**Banc :** [`JustDummies.GenDummy.UnitTests/Sweep/`](../../../../JustDummies.GenDummy.UnitTests/Sweep/)
...
[`GuardCorpus.cs`](../../../../JustDummies.GenDummy.UnitTests/GuardCorpus.cs) réunis ont produit **aucun
...
| 5 | un tirage produit une valeur, **ou** est refusé par `DummyGenerationException` | une trouvaille |
...
Chacun des 352 échecs de tirage du balayage d'août était une `DummyGenerationException` — la bibliothèque
...
[`SweepDefects.cs`](../../../../JustDummies.GenDummy.UnitTests/Sweep/SweepDefects.cs), corrigées ni l'une
ni l'autre dans le changement qui a posé le banc.

**`cardinality-hint-lost-through-as` (55 formes).** `Dummy.SetOf(Dummy.Boolean())` plafonne l'ensemble à deux
éléments, parce qu'`DummyBoolean` porte `ICardinalityHint<bool>` et qu'`DummySet` le lit (ADR-0004).
`Dummy.SetOf(Dummy.Boolean().As(value => (bool?)value))` non : `DummyExtensions.As` rend un
`DerivedDummy<TResult>` qui porte la source aléatoire et la reproductibilité de ce qu'il enveloppe, et rien
...
**`nested-collection-loses-its-declared-interface` (48 formes).** `Dummy.SetOf(…)` est typé
`IDummy<HashSet<T>>` et `Dummy.ListOf(…)` `IDummy<List<T>>`, donc une collection *de* l'un de ceux-là porte le
...
théorie ordinaire dans `JustDummies.GenDummy.UnitTests`. Elle ne peut pas trouver ce que le produit trouve.
...
JUSTDUMMIES_SWEEP=1 dotnet test JustDummies.GenDummy.UnitTests    # le produit entier
```
...
* `JustDummies.GenDummy.UnitTests/Sweep/sweep-baseline.tsv` — **versionné**, et vérifié par le balayage
  complet.
