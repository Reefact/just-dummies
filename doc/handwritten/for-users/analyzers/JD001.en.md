...
`Dummy.Reproducibly` takes a synchronous `Action`. An `async` lambda bound to it becomes `async void`: the body runs to its first `await`, then its continuation — every assertion after that `await` — runs *after* the call has already returned, and the exception escapes the reproducible scope entirely. The test passes green even though the body failed.

Pass the asynchronous body to `Dummy.ReproduciblyAsync(Func<Task>)` and `await` it. For a synchronous body, keep `Dummy.Reproducibly(() => { ... })`.
...
public void A_20_percent_discount_takes_a_fifth_off_the_order() {
    Dummy.Reproducibly(async () => {          // JD001: the async body runs as 'async void'
