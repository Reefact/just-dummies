...
`Dummy.ReproduciblyAsync` returns a `Task` that faults with the body's exception. Discarding it — as a standalone statement, or via `_ =` — lets a failing test pass green, because the failure is never observed. `await` the returned task.
...
public void A_20_percent_discount_takes_a_fifth_off_the_order() {
    Dummy.ReproduciblyAsync(async () => {     // JD002: the returned task is discarded
...
```csharp
_ = Dummy.ReproduciblyAsync(async () => { /* ... */ }); // JD002: the returned task is discarded
```
