...
🌍 **Languages:**  
🇬🇧 English (this file) | 🇫🇷 [Français](./JD007.fr.md)
...
public class OrderTests {
    ...
    public OrderTests() {
        _reference = Dummy.String().StartingWith("ORD-").Generate();   // JD007: drawn before the seed is pinned
    }
...
* A draw from an isolated `Dummy.WithSeed(...)` context: that context is unaffected by the ambient scope by design, so reporting it would be wrong.
* A generator reached through a local, a field or a parameter rather than written inline from `Dummy`. The rule only reports a chain it can prove starts at the ambient source, which under-reports rather than misfiring.
