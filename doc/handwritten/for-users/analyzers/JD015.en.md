...
The constants written at the call site admit no value, so the chain throws a `ConflictingDummyConstraintException` the moment the arrange line runs — either because the anchored fragments cannot fit the declared length, or because a declared character constraint admits none of the values a `OneOf(...)` supplies.
...
> `Dummy.String().WithLength(3).StartingWith("ORD-")` conflicts because the prefix needs four characters, while `Dummy.String().WithLength(12).StartingWith("ORD-")` is valid; the call site and the static types are identical in both.
...
```csharp
Dummy.String().WithLength(3).StartingWith("ORD-")                        // the prefix needs 4 characters
Dummy.String().WithMaxLength(6).StartingWith("SKU-").EndingWith("-EUR")  // 8 characters into a cap of 6
Dummy.String().AlphaNumeric().OneOf("ORD-1", "ORD-2")                    // the family allows neither value
Dummy.String().OneOf("abc", "def").InUpperCase()                           // the casing allows neither value
Dummy.String().StartingWith(" ").WithLength(1).NotBlank()                // the prefix fills it, and carries no non-blank character
Dummy.String().WithLength(0).NotBlank()                                  // nowhere at all to put one
Dummy.String().WithoutNumeric().InLowerCase().OneOf("1", "A")            // neither alone empties the pool; together they do
```
...
Dummy.String().OneOf("ORD-1", "ORD-2")                                   // drop the family: the values are yours
Dummy.String().AlphaNumeric().OneOf("ORD1", "ORD2")
Dummy.String().StartingWith("A").WithLength(1).NotBlank()                // the prefix already carries one
Dummy.String().StartingWith(" ").WithLength(2).NotBlank()                // one filler position is left, which is all it asks
Dummy.String().WithoutNumeric().InLowerCase().OneOf("1", "A", "a")       // "a" survives both
```
...
This is the arithmetic the generator performs, and [JD030](JD030.en.md) reads the same floor from the same place, so the interval one names and the budget the other enforces cannot disagree.
