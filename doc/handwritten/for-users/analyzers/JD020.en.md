# JD020: SharedStaticDummyContext
...
A static `DummyContext` looks maximally deterministic — a literal seed, right there in the source — and is not. `DummyContext`'s own documentation states the hazard:
...
```csharp
private static readonly DummyContext Context = Dummy.WithSeed(1234);   // JD020
```
