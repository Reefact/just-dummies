...
<!-- jd:allow=JD033 -->
```csharp
DummyString reference = Dummy.String().AlphaNumeric().StartingWith("ORD-").WithLength(12);    // cannot draw the '-'
DummyString batch     = Dummy.String().WithoutNumeric().StartingWith("ORD-1").WithLength(12); // removes the '1'
DummyString slug      = Dummy.String().InUpperCase().StartingWith("ord-").WithLength(12);       // cannot draw the 'o'
```
...
<!-- jd:allow=JD015 -->
```csharp
DummyString digits = Dummy.String().Numeric().StartingWith("123").WithLength(12);    // the family draws every character
DummyString plain  = Dummy.String().StartingWith("ORD-").WithLength(12);             // no family: the draw is all of ASCII
DummyString cased  = Dummy.String().InUpperCase().StartingWith("ORD-").WithLength(12); // a casing rules letters; '-' is not one
DummyString pooled = Dummy.String().AlphaNumeric().OneOf("ORD-1");                   // a value set: see below
```
...
Once `OneOf(...)` is declared there is no filler: the caller supplies the whole value, so nothing is drawn beside the literal and "appears only where you wrote it" has no subject. A pooled value a constraint refuses is not kept-as-written at all — it is removed from the pool, which is a different fact reported at a different severity. [JD029](JD029.en.md) names the values a constraint removed while the chain still draws; [JD015](JD015.en.md) reports it as a 🟠 Warning once **every** value is gone, because the chain then throws rather than narrows.
...
Because the fact it carries is fixed by the library, not by taste. [ADR-0080](../../for-maintainers/adr/0080-admit-a-rule-that-names-a-resolved-ambiguity.md) admits such a rule on exactly two grounds: the library names a shorter form exactly equivalent by construction — that is [JD031](JD031.en.md) — or two of the chain's own declarations contradict each other about the same characters and a recorded decision settles which governs, which is this one. Both are checkable against a source file rather than argued.
