...
|------|----------|---------|-------------|
| [JD001 AsyncBodyPassedToReproducibly](JD001.en.md) | 🔴 Error | on | An async lambda is passed to the synchronous Dummy.Reproducibly(Action); bound to an Action it becomes async void and its failures never fail the test. Use Dummy.ReproduciblyAsync and await it. |
| [JD002 DiscardedReproduciblyAsyncResult](JD002.en.md) | 🔴 Error | on | The task returned by Dummy.ReproduciblyAsync is discarded (a bare statement, or `_ =`); the body's failures are lost. Await it. |
| [JD003 AwaitableBodyPassedToReproducibly](JD003.en.md) | 🔴 Error | on | A synchronous lambda whose body drops a task, or an async void method group, reaches Dummy.Reproducibly; the scope returns before the assertions run, and CS4014 does not fire. |
| [JD004 DiscardedSeedingResult](JD004.en.md) | 🔴 Error | on | The handle returned by Dummy.UseSeed is discarded, leaving the seed pinned for whatever runs next — or Dummy.WithSeed is called for effect, which pins nothing at all. |
...
| [JD021 BlankReplaySnippet](JD021.en.md) | 🟠 Warning | on | Dummy.UseSeed is given a blank replay snippet, which the guard rejects — from an adapter hook, failing the whole suite. |
...
| [JD012 GeneratorPooledAsValue](JD012.en.md) | 🟠 Warning | on | Dummy.OneOf is given generators, inferring a pool of recipes; drawing from it yields a recipe rather than a value. |
| [JD013 HeldCollectionPassedToOneOf](JD013.en.md) | 🟠 Warning | on | A held collection passed to Dummy.OneOf binds T to the collection type, making a pool of one; Dummy.ElementOf draws from its elements. |
...
| [JD030 UndeclaredStringLength](JD030.en.md) | 🔵 Info | on | A `Dummy.String()` chain that declares no length, so it draws the whole default spread — 0 to 1024 characters. Names the remedy where you can act on it. |
