...
L'avertissement ci-dessus porte sur le `Any.OneOf<T>` **de premier niveau**, où `T` est inféré et peut
silencieusement se lier à la collection elle-même. Le `OneOf` d'un **constructeur typé** n'a aucun `T`
à inférer : il accepte les deux formes, et il n'y a rien à rater :
...
`ElementOf` renvoie un générateur de vivier qui ne porte que `Except`/`DifferentFrom` ; un `OneOf`
typé renvoie le constructeur, si bien que les contraintes propres au type continuent de resserrer
l'ensemble :
...
Tout générateur qui offre `OneOf` offre les deux formes — cette uniformité est la règle, et un test
tient la surface à celle-ci. Une conséquence mérite d'être connue : une `string` est un
`IEnumerable<char>`, donc sur `Any.Char()` la forme séquence en accepte une directement, et elle se
lit comme les caractères qu'elle énumère :
