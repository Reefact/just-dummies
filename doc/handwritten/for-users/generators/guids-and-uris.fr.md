...
🌍 **Langues :**  
🇬🇧 [English](./guids-and-uris.en.md) | 🇫🇷 Français (ce fichier)
...
Guid empty    = Dummy.Guid().Empty().Generate();        // toujours Guid.Empty
...
manque ? » se lit mieux en `Dummy.Guid().Empty()` qu'en littéral, car il reste dans le même vocabulaire
que ses voisins.
...
`Dummy.Uri()` est le point d'entrée et, non contraint, il couvre tout l'espace d'URI sûr — une URI web,
...
```mermaid
flowchart TD
    accTitle: Les sortes d'URI que Dummy.Uri() sait tirer
    accDescr: Dummy.Uri() se ramifie vers Web pour http et https, WebSocket pour ws et wss, Ftp pour ftp, Mailto pour mailto, et Relative pour un chemin tel que /a/b/c.
...
Uri anyPort  = Dummy.Uri().Web().WithPort().Generate();                   // un port, non précisé
...
```csharp
Uri socket = Dummy.Uri().WebSocket().Generate();                    // ws:// ou wss://
...
qu'un test voulait dire — d'où son propre diagnostic, [JD026](../analyzers/JD026.fr.md). C'est la
