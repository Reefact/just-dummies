...
confused them. Writing `$"{Dummy.Int32()}"` compiles perfectly and yields the string
`"JustDummies.DummyInt32"`. That is diagnostic [JD005](../analyzers/JD005.en.md), and it exists
precisely because nothing else would have caught it.

```mermaid
flowchart TD
    accTitle: Why a generator is a recipe rather than a value
    accDescr: Dummy.Int32() returns a generator of any int. Between(1, 100) returns a further generator, and MultipleOf(5) another again. Calling Generate() twice on that last generator yields two different values, 45 and 70.
    F["Dummy.Int32()"] -->|"returns"| G1["generator<br/><i>any int</i>"]
...
## `IDummy<T>` is the seam everything composes on

Every generator implements `IDummy<T>`, whose only member is `Generate()`. That single interface is
...
It is also the currency of the composition API: `Dummy.ListOf`, `Dummy.Combine`, `.As(...)` and
`.OrNull()` all take and return `IDummy<T>`. See [Composition](./composition.en.md) for what that
makes possible.
...
`Dummy.Int32().Between(1, 100).MultipleOf(7)` picks from the multiples of seven in that interval; it
...
<!-- jd:allow=JD023 -->
```csharp
// Throws ConflictingDummyConstraintException — the message names both bounds.
...
flowchart LR
    ...
    accDescr: The declared constraints are asked whether they admit a value. If they do not, a ConflictingDummyConstraintException names both sides. If they do, a value satisfying all of them is built, and that is the drawn value.
    D["declared constraints"] --> C{"do they admit<br/>a value?"}
    C -->|no| X["ConflictingDummyConstraintException<br/><i>naming both sides</i>"]
...
* **No whole-object graph.** There is no `Dummy.Object<T>()` that reflects over your type and fills it
