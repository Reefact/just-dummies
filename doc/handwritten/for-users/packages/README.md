...
| --- | --- | --- |
| [`JustDummies`](./justdummies.en.md) | the library, with its 32 rules bundled in | **Yes** — this is the product |
...
| [`JustDummies.DiagnosticCatalog`](./justdummies-diagnosticcatalog.en.md) | the `JD001`–`JD032` rules as compile-checked constants | Only to suppress a rule without a string literal |
...
```mermaid
flowchart TD
    L["JustDummies<br/><i>the library</i>"] -->|"bundles"| A["32 rules<br/><i>analyzers/dotnet/cs</i>"]
